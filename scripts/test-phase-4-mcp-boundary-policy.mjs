...
const checkerPath = resolve(repositoryRoot, "scripts/check-phase-4-mcp-boundary.mjs");
const temporaryRoot = await mkdtemp(resolve(tmpdir(), "archflow-phase-4-mcp-boundary-"));
...
function assertRejected(label, expected) {
  ...
  assert.notEqual(result.status, 0, `${label} must fail the Phase 4 MCP boundary`);
...
try {
  ...
  console.log("Phase 4 MCP boundary mutation checks rejected static, type, side-effect, dynamic, re-export, and private-path violations.");
} finally {
