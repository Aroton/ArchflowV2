...
function parseRepositoryRoot(args) {
  ...
  if (args.length !== 2 || args[0] !== "--root") {
    throw new Error("usage: node scripts/check-mcp-sdk-boundary.mjs [--root <repository-root>]");
  }
...
try {
  ...
  failures.push("src must exist so the MCP SDK production boundary can be checked");
  sourceFiles = [];
}
...
if (failures.length > 0) {
  ...
  console.log(`MCP SDK boundary passed for ${sourceFiles.length} production TypeScript files; SDK imports are isolated to ${adapterPath}.`);
}
