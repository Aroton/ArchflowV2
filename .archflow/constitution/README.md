...
To add a repository-specific human boundary, add a numbered rule with an observable `review_trigger`. For example:
...
For an existing repository, compare its rules with the new templates and selectively apply the intended changes on its policy/base branch, incrementing changed rule versions. Preserve custom rules and triggers; do not use a forced scaffold refresh to adopt defaults. A requested constitution update authorizes committing that scoped change. Future tasks use the resulting committed policy; existing tasks retain their original policy and any pending gate. Installing a newer bundle alone does not adopt repository policy.
