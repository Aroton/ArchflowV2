...
- The fallback only retries and accepts; it never rewrites recorded values. Comparing sites — the kernel, the local envelope, and the gate-reentry *replay validations* — keep recording the accepted, possibly legacy value, since the committed state must equal the fingerprint the request claimed; the composition flips to the new one only at no-expected write sites (revision zero, planning-restart landings, and gate-reentry landings, whose recomputation can never equal the pre-reentry recorded value because the reentry call's step and rubric digest differ), so legacy tasks converge at those sites rather than on every transaction.
...
- New submission kind `review-dispatch` with one optional field `route_override: RouteOverrideDeclaration`, accepted by the `review` action (whose `expected_submission` is currently `"none"`, `src/state/semantic-view.ts:185` — relax `assertSubmissionMatches` for the review action to accept `none` or `review-dispatch`).
...
- MCP root-schema convention is safe: the union stays nested under `action.submission` (`src/contracts/semantic-workflow.ts:311-312`); regenerate `schemas/v1/semantic-workflow.schema.json` via `npm run generate:schemas`.
...
Files: `src/contracts/semantic-workflow.ts` (submission kind + union + enums), regenerate `schemas/v1/semantic-workflow.schema.json`, `src/state/semantic-actions.ts` (submission matching for review + `requestFacts` threading), `src/state/semantic-view.ts` (review instruction mentions the override option), skills `archflow-prd`/`archflow-design`/`archflow-phase-design`/`archflow-phase-impl` (outage paragraph: human reason required, submit with the review offer) + `archflow-init` (config routing/overrides guidance), `test/contracts/skill-contract-canonical.test.ts` + `skill-contract-server-outage.test.ts` pins, new unit/integration tests for submission→facts→evidence threading and operation-digest binding. Success: PRD criterion 4.
