...
|---------------|--------------|------------|----------------|----------------|
| Initialization and canonical repository contracts | REQ-01, REQ-02, REQ-03, REQ-04, REQ-05, REQ-06 | VAL-08, VAL-11, VAL-12 | 1, 9, 10, 15 | Strict assets/schemas, preserving official registration, fixed graph, and documentation agreement establish one v1 authority. |
| Thin phase marshalling and production lifecycle | REQ-07, REQ-08, REQ-09, REQ-10 | VAL-01, VAL-02, VAL-05, VAL-06, VAL-16 | 1, 8, 10, 14, 15 | Normal skills drive persisted boundaries, implementation manifests, current-digest loops, approvals, parent updates, and commit authorization. |
| Review evidence and truthful status | REQ-11, REQ-12, REQ-13, REQ-14 | VAL-02, VAL-05, VAL-06, VAL-08, VAL-12, VAL-16 | 1, 2, 3, 4, 7, 10, 11, 13, 14 | Structured JSON/projections, fixed-point fixtures, and reconciled normal/degraded status distinguish provenance. |
| Constitution and task policy | REQ-15, REQ-16, REQ-17 | VAL-03, VAL-15 | 1, 7, 12, 13, 14 | Explicit approved policy base, trigger corpus, and unresolved-edit rejection fail closed. |
| Waivers, drift, and human authority | REQ-18, REQ-19, REQ-20 | VAL-01, VAL-03, VAL-04, VAL-09, VAL-15 | 5, 7, 12, 14 | Digest-bound decisions, scoped expiry, migration audit, separate verdicts, and upstream reopening preserve authority. |
| Durable state and recovery | REQ-21, REQ-22 | VAL-05, VAL-06, VAL-09, VAL-12 | 2, 3, 4, 5, 11, 13, 14 | Prepared-before-state semantics, fault injection, durable gates, helper reconciliation, and collision-safe restore never infer success. |
| Idempotency and concurrency | REQ-23, REQ-24, REQ-25 | VAL-05, VAL-06, VAL-08, VAL-10 | 2, 3, 4, 5, 11, 12, 13 | Intent/state commitment, local lock/CAS, one-writer Git handoff, manifests, and exact restore cover retries/races without claiming distributed locks. |
| Repository and task isolation | REQ-26 | VAL-07, VAL-10, VAL-11, VAL-13 | 2, 3, 4, 6, 12, 13, 14 | Path guards, narrow pre-init legacy read, and measured sandbox manifests block undeclared access. |
| Fixed MCP boundary and local service | REQ-27, REQ-28 | VAL-08, VAL-13 | 1, 8, 9, 13, 14 | v2 fixtures expose five stdio tools; the separate offline helper is not an MCP tool. |
| Host identity and routing | REQ-29, REQ-30, REQ-31 | VAL-07, VAL-08 | 6, 8, 13, 14 | Real handshakes and exact CLI fixtures bind immutable host/opposite family and reject injected managed context. |
| Fresh structured dispatch | REQ-32, REQ-33 | VAL-02, VAL-07, VAL-08, VAL-13 | 6, 7, 8, 13, 14 | Hashed envelopes, measured runtime/selected-CLI own-auth reads, fresh PIDs, OS isolation, validation, and attestation prove the boundary. |
| Authentication and child lifecycle | REQ-34, REQ-35 | VAL-07, VAL-08, VAL-13, VAL-14 | 6, 9, 13, 14, 15 | Auth/version/policy preflights, environment sentinels, descendant escape tests, and the legal release gate prevent unsafe dispatch. |
| Durable gate semantics | REQ-36, REQ-37 | VAL-01, VAL-03, VAL-09, VAL-10 | 5, 8, 11, 13, 14 | Deterministic IDs, pre-block publication, supplemental-review retry, cancellation, and CAS resolution prove exactly-once decisions. |
| Degraded/manual terminal completion | REQ-38, REQ-39, REQ-40 | VAL-06, VAL-09, VAL-12 | 8, 9, 11, 12, 14 | `archflow-local` and exact fallback templates advance only from validating evidence; loss of both paths stops for repair. |
| Optional cross-client gate review | REQ-41 | VAL-09, VAL-12, VAL-16 | 5, 10, 11, 14 | Caller-known gate IDs avoid blocking deadlock; triage resumes or supersedes deterministically and decline fabricates nothing. |
| Legacy migration | REQ-50 | VAL-17 | 2, 3, 4, 5, 12, 13, 14 | Staged import, no-state initialization union, existing phase mappings, implementation audits, and interruption fixtures prevent auto-promotion. |
...
**Goal**: Establish a buildable server package and the versioned contracts all later phases share.

**Requirements**: REQ-02, REQ-03, REQ-04, REQ-05, REQ-06, REQ-10, REQ-11, REQ-12, REQ-15, REQ-17, REQ-27, REQ-28, REQ-33

**Scope**: As the first Phase 1 action before server implementation, add a visible lineage/supersession banner to `docs/mcp-integration-design.md` identifying it as the originating detailed design and the approved PRD/architecture as normative where they differ; Phase 15 later updates the surrounding documentation while preserving that history. Then add Node/TypeScript/MCP v2 beta package scaffolding and exact lockfile; isolate the beta SDK adapter; define the fixed workflow vocabulary, shipped workflow and constitution assets, normative strict durable JSON Schemas, derived/wrapped Zod MCP contracts, a shared validator-agreement corpus, canonical phase-instance encoding, canonical Markdown renderers, error envelope, license notices, build, lint/typecheck, and protocol/contract fixture harness. This phase does not implement persistent transitions or dispatch.

**Success Criteria**:

- [ ] The project installs from the exact lockfile, typechecks, tests, and builds at the functional Node floor `24.15.0`; CI/release also passes on the current Node 24 LTS security patch (`24.18.0` at design time), independently of the Node-24 typings patch.
- [ ] MCP initialization and `tools/list` fixtures expose protocol `2025-11-25`, exactly the five named tools, and versioned schemas; stdout contains no non-protocol output.
- [ ] Valid workflow/review/state samples round-trip through strict schemas and canonical rendering, while unknown graph vocabulary, invalid verdict/count combinations, and non-reversible phase IDs fail with stable errors. For every overlapping persisted/MCP shape, the same valid/invalid corpus proves Ajv and Zod accept and reject identically.
- [ ] The dependency-upgrade procedure demonstrates that changing the MCP beta pin requires fixture, lockfile, and license review.
...
### Phase 2: Repository Identity, Paths, Schemas, and Canonical Digests
...
**Scope**: Implement Git/worktree and task identity; `.archflow/** -text merge=binary`; path allowlists; task-initialization, document, implementation-output, legacy-import, manual-checkpoint, checkpoint-import, maintenance-record, and snapshot-accounting schemas; exact whole-file `config.yaml` pinning; canonical Git blob/tree-mode and request/input digests; divergence/conflict detection; and secret-scan result contracts. This phase does not implement state mutation or payload restore.
...
- [ ] Valid initialization/document/implementation/import/checkpoint/maintenance samples round-trip through normative schemas; predecessor gaps, unknown fields, digest mismatches, invalid phase IDs, and contradictory accounting fail with stable errors.
...
### Phase 3: Transaction Kernel, Intent/CAS, and Crash Recovery
...
### Phase 4: Snapshots, Implementation Manifests, and Restore
...
### Phase 5: Durable Gates, Waivers, and Manual Decisions
...
### Phase 6: Host Identity, Sandbox, and CLI Dispatch
...
### Phase 7: Constitution Adjudication, Drift, and Review Fixed Point
...
### Phase 8: Five-Tool MCP Assembly and Offline Local CLI
...
### Phase 9: Installer, Initialization, and Host Registration
...
### Phase 10: Normal-Mode Thin Phase Skills and Truthful Status
...
### Phase 11: Manual and Degraded Recovery Workflow
...
### Phase 13: Reliability and Security Matrices
...
### Phase 14: Real-Host E2E and Review-Quality Validation
...
### Phase 15: Release Packaging, Support Matrix, and Documentation Consistency
...
| 2 | Repository Identity, Paths, Schemas, and Canonical Digests | Not Started |
| 3 | Transaction Kernel, Intent/CAS, and Crash Recovery | Not Started |
| 4 | Snapshots, Implementation Manifests, and Restore | Not Started |
| 5 | Durable Gates, Waivers, and Manual Decisions | Not Started |
| 6 | Host Identity, Sandbox, and CLI Dispatch | Not Started |
| 7 | Constitution Adjudication, Drift, and Review Fixed Point | Not Started |
| 8 | Five-Tool MCP Assembly and Offline Local CLI | Not Started |
| 9 | Installer, Initialization, and Host Registration | Not Started |
| 10 | Normal-Mode Thin Phase Skills and Truthful Status | Not Started |
| 11 | Manual and Degraded Recovery Workflow | Not Started |
| 12 | Legacy Upgrade Workflow | Not Started |
| 13 | Reliability and Security Matrices | Not Started |
| 14 | Real-Host E2E and Review-Quality Validation | Not Started |
| 15 | Release Packaging, Support Matrix, and Documentation Consistency | Not Started |
