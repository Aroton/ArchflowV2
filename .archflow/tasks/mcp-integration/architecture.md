...
MCP v2 stdio boundary
        ...
        +--> dispatch coordinator
               ...
               +--> best-effort suppression + generated clean home/temp cwd
...
7. Perform the bounded operation. Phase 13 model calls execute from a non-repository temporary directory with a generated clean home, scrubbed environment, and adapter suppression flags. These are best-effort context hygiene, not an OS-enforced read or process-containment boundary. Model JSON must validate before it can become evidence.
...
A crash before step 9 never implies success. A matching receipt-only crash is resumable by the same intent after the caller refreshes CAS; it is never reported as committed until state authenticates it. Phase 10 reconciliation distinguishes that orphan when the caller supplies the exact current intent and receipt; `state.json`-only status still cannot distinguish it from the unchanged prior revision until Phase 17 adds reconciliation-aware status. Ordinary pre-request reconciliation is bounded to the projections referenced by current state, the active or prepared intent relevant to the request, and the active gate and manual-checkpoint heads; it does not scan retained result, intent, decision, or checkpoint history. A full-chain/history audit is reserved for explicit repair, import, or audit operations. Reconciliation restores the current projections when collision-safe, otherwise stops for the applicable gate or exact repair; it never promotes a prepared/orphan result or model output by inference. Long-lived gates are the exception to holding a lock continuously: gate creation is committed under the sequence above, the lock is released while the request blocks on a decision file, and resolution reacquires the lock and applies a new CAS transition against the same gate ID.
...
Repository initialization and task initialization are distinct. `archflow-init` scaffolds repository assets and host registration only. Its Phase 16 `config.yaml` template, rather than constants in `src/`, supplies the default opposite-family models and effort; a configured override is resolved by phase kind and role, and an absent role needed for dispatch is a classified non-launching error. Before a task can start, the workflow and constitution assets must exist in an immutable human-approved commit; ArchFlow never creates that commit automatically. Fresh task creation through its skill and `archflow-local` stages a `task-initialization` manifest, and `archflow_state` adopts it directly or through a closed manual checkpoint import whose first checkpoint contains that manifest. Both routes begin at no-state/revision-sentinel-0 and establish initialized/revision-1 authority before adopting any later checkpoint. Missing, uncommitted, mismatched, or task-locally modified policy bases fail closed.
...
src/
  ...
  contracts/
    ...
    hosts.ts                      # pure MCP clientInfo -> host identity derivation
  ...
  state/
    ...
    constitution.ts               # immutable policy-base registry + task-edit detection
    fingerprint-readers.ts        # production canonical input-fingerprint readers
    evidence-results.ts           # retained evidence snapshots and reload
  ...
  review/
    ...
    counter-review.ts
    adjudication.ts
    fixed-point.ts                # durable evidence currency and re-entry decision
  dispatch/
    routing.ts                    # family derivation and configured route validation
    workspace.ts                  # generated home/temp cwd and allowlisted environment
    process.ts                    # bounded child lifecycle and output canary scan
    cli.ts                        # compile-time-known Claude/Codex adapters and attestation mint
  init/
    assets.ts
...
.archflow/
  ...
  tasks/<task>/
    ...
    results/sha256/<result-digest>/
      manifest.json                    # embeds the exact validated source artifact
...
| State revision | Schema version, monotonic revision, phase instance, step/status/attempt, authoritative result refs, approvals, open gate, waivers, and optional `planned_final_phase` | `state.json` is normal-mode authority and is written last. A successful step references validating artifacts with matching digests. Design approval derives the optional final phase only from consecutive exact `### Phase N: Name` headings starting at 1; malformed or absent plans fail that gate, while the exact `<!-- archflow:phase-plan:open-ended -->` marker with no phase headings intentionally leaves the field absent. |
| Declared-input fingerprint | Hash of versioned workflow, the immutable whole-config digest, pinned constitution, phase instance, and explicitly declared inputs | Recomputed by the server from the canonical identities below. Non-produce steps carry no declared inputs and therefore share one fingerprint across self-review, counter-review, triage, and adjudication; artifact and approved-upstream freshness remain explicit installation/currency checks. Whole-config binding is deliberate: because config cannot change within a task, it never causes partial in-task invalidation. |
| Result snapshot | Content-addressed manifest embedding the exact validated source artifact, canonical Git identities for commit-reachable tracked outputs, and bounded copied payloads everywhere else | Immutable and task-local. The embedded artifact keeps implementation evidence and its digest resolvable after the originating request. Canonical Markdown/files are replaceable projections of a referenced snapshot; copied payload accounting enforces the limits below. |
...
| Implementation-output manifest | Versioned tagged artifact binding declared source/task paths and add/modify/delete/rename operations; file type, tree mode, and canonical Git blob identities; before/after digests; base plus declared-path-scoped index/worktree identity; parent-document outputs; exact canonical diff digest; snapshot/restore targets; bounded raw payload bytes where required; and undeclared-change scan | It is the complete phase-implementation and commit-authorization subject. Outputs reuse Git objects only when the exact object and mode are proven reachable from a retained commit; uncommitted or index-only bytes use bounded copied payloads. Restore is collision-safe in normal and manual mode. |
...
| Manual milestone checkpoint | Immutable schema-versioned task/phase/step/status, phase-instance attempt counter, declared-input fingerprint, authoritative result/snapshot and projection digests, current evidence chain, decisions/gates/waivers, predecessor checkpoint digest/revision, and degraded assurance | In manual mode, only snapshots reachable from the latest valid predecessor-linked checkpoint are authoritative. Prepared/uncheckpointed files are not. A retry increments the phase-instance counter and later steps carry it forward; only a new phase instance resets it to 1. Normal and legacy initialization each begin with a checkpoint. |
| Review evidence | Validated source JSON retained as a tagged evidence artifact and represented by canonical Markdown with subject/input digests, stable findings, verdict/counts, assurance and execution metadata | `evidence_digest` and the result manifest's `artifact_digest` are the canonical JSON payload digest; projection `payload_digest`/`content_digest` is the rendered-byte digest. Validation and rendering happen before commit, but authority links are derived only from a reloaded retained manifest plus the authenticated committed transaction outcome. Server-attested, agent-declared, and degraded provenance cannot be upgraded by prose or projection bytes. |
| Gate | Unique ID, task/phase/kind, subject/context digests, allowed decisions, active status, optional waiver-origin gate ID, and a frozen-state digest | At most one active gate per task. The optional origin ID distinguishes a waiver gate from an ordinary gate of the same kind. The frozen-state digest covers the canonical open revision with only `open_gate` excluded to avoid a digest cycle; this lets a request archive survive a pre-publication crash and later revision while binding the required absence of `committed_intent` and every other field after the gate opens. Timeout, cancellation, disconnect, or missing decision leaves it pending. |
...
| Waiver | Pinned rule version, recorded narrow scope, task/subject/evidence digests, explicit grant/denial and expiry | The durable reference repeats the archived decision's exact scope so a grant can resume only that rule/scope tuple. It never amends policy, crosses tasks, or silently follows a changed subject. |
...
**Phase 17 amendment to review and upstream identity.** Review, adjudication, gate, and status `subject_digest` values identify the canonical source artifact retained by the current successful produce result: they are the retained result manifest's `artifact_digest`, not a digest of a replaceable projected file. The selected projection's byte digest remains a separate freshness and selector check against that retained manifest; it is not an alternative authority domain. Adjudication likewise records each approved upstream by the upstream produce manifest's `artifact_digest`, and artifact approval and later currency checks use that same domain. The current projected upstream document bytes remain dispatch material only, so revising an upstream requires a new retained produce result and approval rather than silently changing the already-approved identity.

Canonical Git identity is part of the contract, not an ambient checkout accident. Pinned committed inputs bind the exact Git blob bytes and tree modes from the pinned tree. Repository attributes mark `.archflow/** -text merge=binary`, so its tracked authority bytes neither pass through end-of-line conversion nor receive text merges. Regular-file identities use the blob produced after the path's Git attributes/clean conversion; symlink identities hash target bytes without path conversion. A result may use `git-object` storage only when the exact OID and mode resolve from a commit/path recorded in the retained source artifact and that commit is proven reachable from the current task-branch tip at creation and every read/restore. Merely naming a commit, observing an index entry, finding an object, or creating one with `hash-object -w` is not retention. Every other surviving output byte — including ordinary uncommitted tracked implementation output — is copied as a bounded raw payload. This contract is invariant across `core.autocrlf`, working-tree line endings, `core.fileMode`, executable-bit support, and symlink checkout capability; unsupported materialization fails with guidance instead of changing the identity.

**Phase 11 amendment to the Phase 7 snapshot field.** The Phase 7 `DocumentArtifactV1` comment described `snapshot_digest` as the retained result the document projects. That interpretation would require an agent-supplied artifact to predict the server-minted result-manifest digest and creates a cycle. From Phase 11 onward, `snapshot_digest` names only the domain-separated canonical declared-output snapshot: the sorted final present/absent states of declared targets. `result_digest` alone names the retained result manifest. Phase 11 updates the shipped comments and verifies document and implementation snapshot digests from real bytes and normalized declared operations.

Copied raw payload is capped at 25 MiB per result and 250 MiB per task, counted from validated stored bytes before projection. Exceeding either limit returns stable non-advancing `SNAPSHOT_LIMIT` with the exact offending paths, current/cap byte counts, and safe guidance to reduce or commit the declared output; payload is never truncated and never moves to external hidden authority. Before any snapshot or manual checkpoint is projected into Git-tracked `.archflow/`, ArchFlow scans the complete candidate projection for secrets and fails closed with path-safe remediation. Guidance explicitly warns that later deletion of `.archflow/` does not erase secrets from branch history.

Retention preserves every currently authoritative generation and the complete decision/review/manual-checkpoint chain through task lifetime. Maintenance uses a schema-specific caller-bounded walk, not a general graph engine: validated current state, supplied checkpoint chains, supplied resumable receipts, and supplied decision/review evidence provide result references; those references name manifests; manifests name payloads. Missing or incomplete supplied roots fail closed, and Phase 15 owns complete enumeration. An explicit human-approved maintenance operation may prune only unreferenced diagnostic attempts and superseded non-authoritative payloads after that bounded proof preserves deterministic rerun, receipt-only resume, and every checkpoint chain. It writes an immutable maintenance record containing every deleted digest, byte count, and human reason before deletion; it cannot prune manifests, current generations, authoritative evidence, or any payload still needed for replay/restore.
...
- A retained adjudication is assessed with the same pinned rule registry used to select its ordered obligations: constitution failure/uncertainty, the first current material upstream, then review trigger/uncertainty. The next action remains `adjudication-gate` for the first unsatisfied obligation until its exact advancing approval or exact live waiver exists; authority for one category never discharges another, and advancement occurs only after every applicable category is satisfied. Material resolution still forces re-entry and fresh adjudication before another material upstream can gate. A result-commit/gate-publication crash and any non-advancing closure cannot authorize advancement.
- `attempt` counts attempts consumed by the whole phase instance. It carries across forward steps, increments on same-step retry or fixed-point re-entry, and resets only for a new phase instance. Exhaustion is evaluated at re-entry rather than on every forward move.
...
- The pinned constitution is read from the immutable base commit using the sorted `<NN>-<name>.md` path/OID registry; `README.md` is outside the digest but any constitution-directory change still triggers edit detection. A rule with no declared `enforced_by` may pass with an empty list; a declared mechanism cannot pass on model-asserted evidence and gates until server-corroborated evidence or an exact waiver exists. A task-branch constitution-edit gate may request revert/abort or base-branch amendment work, but cannot approve the edit. Base amendment is external to the frozen task policy basis, so completion requires a new/migrated task and this task ends through abort.
...
- Retry-effect gate resolution is an authenticated second entry to the transition kernel: `revise`, `revise-current`, and `retry-once` validate their exact predecessor, derive a fresh produce-entry fingerprint through the handler-supplied gate callback, and land at `produce/running` with `attempt + 1`. Redirect, constitution-edit, rejection, abort, and cancellation decisions do not enact that transition.
...
- `review-evidence` wraps validated self-review evidence for durable `archflow_state` installation. Its canonical payload digest is the evidence and manifest artifact identity; its Markdown bytes are a separate review projection.
- `triage` wraps exact current-review dispositions for durable `archflow_state` installation under the same payload-versus-projection digest split.

The seven-member union makes document, code-output, migration, self-review, and triage state representable without adding a tool or inventing a migration phase instance. Adjudication evidence uses the same retained-result artifact shape but enters only through `archflow_adjudicate`, not through this MCP union.

Gate IDs are deterministic and caller-known from canonical task identity, `intent_id`, and request digest. A skill can therefore present the REQ-41 prompt with the exact gate ID before invoking the blocking call. `archflow_gate` publishes and commits the request before it waits. The gate-open revision writes no success receipt and clears `committed_intent`, because a pending gate is not a successful outcome and retaining the prior binding would make current-state reconciliation falsely invalidate the open revision. The other client emits versioned structured JSON, not an authoritative Markdown file. From a second terminal, the human runs `archflow-local` in normal or manual mode; it validates the JSON, binds the exact task/gate/subject/input digests, canonically renders it, and atomically renames the complete projection to `reviews/<phase-instance>.gate-counter.<gate-id>.md`. The complete projection is only a wake-up interface: `resolve_supplemental_review` must return server-derived parsed evidence from durable authority, and the gate service recomputes its payload digest and authenticates every gate/task/phase/role/subject/fingerprint/assurance/family binding before retaining it. Caller-authored ledger entries and Markdown bytes confer no authority; an explicit decline remains resolver-free. The blocking server uses a poll-only 500 ms loop; filesystem notification is deliberately omitted because polling is the portable primitive across native Linux, macOS, and WSL2-mounted filesystems. This poll loop has no deadline of its own: Phase 15 owns the retained-evidence resolver and tool cancellation wiring, and Phase 16 owns the host timeout constants and registration.

While a gate is pending, its closed legal transition set is supplemental-review ingestion, triage of that review, an explicit decline or gate decision, explicit cancellation, or supersession after an accepted change/changed subject. If an untriaged supplemental review arrives, the wait ends with non-success `SUPPLEMENTAL_REVIEW_REQUIRED` while the gate remains pending. The registry correctly marks that result non-retryable as issued: the same intent becomes retryable only after the required triage, using refreshed CAS and unchanged evidence. The resumed orchestrator rejects current-digest findings and retries the same intent, or accepts a change, closes the gate as non-advancing `superseded`, re-enters the fixed-point pipeline, and later creates a fresh intent and deterministic gate for the new subject. Explicit decline fabricates no review and allows the otherwise-valid decision path.
...
Each Phase 13 dispatch creates a fresh, non-resumed child in a temporary directory outside the repository. The counter-review envelope represents only artifact bytes, rubric, fixed result schema, and minimal phase/execution identity; it is canonical, versioned, size-bounded, hashed, and retained as declared input evidence. Adjudication envelope construction remains Phase 14 work. The child receives a generated clean home, a scrubbed allowlist environment, and explicit CLI suppression flags. Dispatches are serialized through one in-process queue because the published Codex subscription-auth guidance assumes one machine or a serialized job stream per `auth.json` copy.

This is best-effort context hygiene, not an enforced or proven isolation boundary. Phase 13 has no `SandboxProvider`, capability proof, probe gate, or environment declared unsupported for lack of isolation. Codex's `--sandbox read-only` constrains model-generated shell commands only; the Codex parent retains filesystem and network access. `--ephemeral` is also not a no-persistence guarantee: Codex can download remote plugins/system skills into `CODEX_HOME`, global `AGENTS.md` loads cannot be disabled from an existing home, and project skill descriptions can still be discovered unless instruction inclusion is disabled. A generated clean `CODEX_HOME` is therefore the hygiene lever. The deny-all Codex execpolicy spike is closed as infeasible because its parser rejects an empty pattern and has no wildcard first token. Claude's `--tools ""` is stronger, but managed hooks can execute shell and inject context independently and survive `--safe-mode`. Suppression flags can drift, especially Codex's denylist against its changing tool/feature registry, and the actual Codex tool surface cannot currently be inspected through `codex debug prompt-input`.

Consequently, the earlier Phase 13 `SandboxProvider` obligations and OS-isolation success claims are unmet, and Phase 13 provides only the counter-review half of REQ-32. REQ-32 and VAL-07 are not fully satisfied. The owner explicitly accepts this limitation based on extensive manual counter-review experience, including the possibility of slightly worse results, and has ruled out enforcement/probe machinery for this phase. The phase still scrubs provider key/routing variables, uses a generated home and non-repository cwd, bounds input/output, and scans planted canaries by exact substring; generic secret detection over child output is not claimed. Managed-policy presence is conservative `CliPreflight` telemetry for Phase 15 to retain with the dispatch attempt, not a field in the Phase 2-frozen review attestation. ArchFlow does not read, copy, inject, prompt for, persist, or log credential values, but auth preflight proves only the logged-in bit under the same scrubbed child environment and does not distinguish subscription from every unsupported auth mode.

The Claude adapter uses explicit safe-mode/tool/MCP/slash-command/settings/persistence suppression and inline schema-constrained output. The Codex adapter uses explicit ephemeral, ignore-user-config/rules, read-only, project-document/skill suppression, output-schema, and feature-disable flags. Both are version-gated, spawn through `node:child_process` with `shell: false`, use cancellation/timeout/output bounds, and classify stable failures. A future release-hardening phase may revisit real containment; for that successor, `bwrap --unshare-pid --die-with-parent` is the promising Linux primitive, while `unshare --pid --fork` without `--kill-child` orphans the namespace when the wrapper dies. Any mount design should bind the credential file rather than the large state directory; cgroups are telemetry only where delegation is unavailable, and the measured approximately 9 ms `bwrap true` startup is not proof of a working CLI mount set.

### Initialization, Manual Operation, and Upgrade

`install.sh` installs one offline host-neutral bundle with stable PATH-resolved `archflow-mcp` and `archflow-local` launchers, then copies the same `skills/` source to both clients. Claude registration must run `claude mcp add --scope project archflow -- archflow-mcp`; the official command creates/updates the PRD-required project `.mcp.json`, which is shared and committed with the repository. Project-scope presence, command collision, and the persistent timeout are authenticated from that parsed file; `claude mcp get` is diagnostics only because a local or user entry can mask it. The project entry registers the stable command name and `timeout: 3600000`, never an absolute home/install path. Codex has no official project-scoped registration command, so init never calls `codex mcp add`: it owns only one marker-delimited `[mcp_servers.archflow]` block in the repository's `.codex/config.toml`, preserves every unrelated byte, and treats any same-name entry outside that block as a refusal. Init never replaces whole JSON/TOML files or claims a generic rewriter.

Claude Code's supported project `timeout` field is the persistent authority; on the supported floor it also raises that server's stdio idle window, so init does not write `MCP_TOOL_TIMEOUT`, edit a shell profile, or launch a new host to prove environment inheritance. Codex project activation is proven only when `codex mcp get archflow --json` resolves the managed command and the configured `startup_timeout_sec: 30` and `tool_timeout_sec: 3600`; a global same-name entry with absent timeouts is not project trust. Init detects registration collisions, invalid/untrusted configuration, unsupported runtime/CLI versions, and missing launcher capability, and reports both host approval boundaries plus managed-instruction and unenforced-isolation limitations. Machine-specific executable locations stay only in installation/PATH resolution and never enter `.mcp.json`, `.codex/config.toml`, or portable task state.
...
| Evidence authority timing | Treat canonical rendering as authority; mint links before state; derive links from committed retention | Validate/render before commit, then mint authority only from a reloaded retained manifest and authenticated transaction outcome | Review files remain reconstructible projections. Result, receipt, and revision facts cannot be caller-invented or inferred from a prepared snapshot. |
| Evidence identity | Rendered-file digest; wrapper digest; canonical payload digest | Canonical evidence-payload digest, with rendered bytes separately digested as a projection | Renderers embed the evidence digest, so using rendered bytes as evidence identity is cyclic. The payload digest also agrees with the result manifest's `artifact_digest`. |
| Review isolation | Fresh prompt only; enforced sandbox now; best-effort hygiene now with release proof deferred | Phase 13 uses suppression flags, a generated clean home, scrubbed environment, and non-repository temp cwd; it does not claim an OS boundary | Fresh process identity and hygiene reduce contamination but do not prevent absolute-path discovery or managed injection. REQ-32/VAL-07 remain partially unmet. |
| Sandbox dependency | Roll OS-specific isolation; immediately adopt a tool; adopt none in Phase 13 | Record corrected package/license evaluation and retain containment as a future release blocker | `@anthropic-ai/sandbox-runtime` aggregates rather than vendors bubblewrap/socat, but its beta API, hard socat prerequisite, macOS reaper gap, and notice issue make it unsuitable. |
| Dispatch concurrency | Concurrent children; per-auth-copy workers; serialized in-process queue | Serialize dispatches | Matches the published single-consumer condition for subscription-authenticated Codex automation without building credential-copy machinery. |
...
| Adjudicator fallback | Automatic same-family degraded result; automatic refusal; checkpoint-backed manual result | Automatic same-family dispatch is refused; only the checkpoint-backed manual path can be degraded | Automatic dispatch lacks the checkpoint authority required by the degraded assurance arm and must not fabricate it. |
...
| Retry authorization | Decision trace interpreted later; direct state mutation; guarded kernel re-entry | Gate closure validates the predecessor and re-enters through the transition planner with a server-derived fingerprint | `revise`, `revise-current`, and `retry-once` become one-shot durable effects without adding a decision index or bypassing legal movement. |
...
| Snapshot bounds and retention | Unbounded payload copies; hidden external store; existence-only Git-object reuse; commit-reachable Git objects plus bounded payloads | Reuse only objects proven reachable from retained commits, copy and cap every other required byte, retain authority/evidence, and allow recorded pruning of unreachable non-authority only | Bounds growth without truncation, hidden authority, or restore promises that depend on garbage-collectable loose objects. |
...
- Adapter fixtures use fake CLIs for every stable failure class and version behavior. Phase 13 fixtures verify the logged-in bit under the scrubbed child environment, model/family identity, explicit suppression flags, fresh PID/session, structured output, cancellation, bounded channels, and best-effort termination without persisting secret values.
- Phase 13 canary tests plant known values and exact-substring scan child output and diagnostics, including a leaking negative control. They demonstrate the hygiene regression check, not absence of filesystem-capable tools, managed injection, generic secrets, or escaped descendants. Future release-hardening may add OS-containment black-box tests; until then release criterion 1 below remains unmet rather than attributed to Phase 13.
- Phase 14's deterministic fake-CLI corpus seeds substantive defects, clean controls, trigger uncertainty, declared-mechanism fail-closed behavior, aligned/incidental/material drift, repeated rewrites, and exhaustion. Phase 21 runs the real-host quality evaluation and owns choosing and explicitly approving release thresholds from observed results; finding count is not the metric.
...
1. A sandbox provider/OS combination has current black-box proof and an explicitly acceptable license. Phase 13 does not satisfy this criterion and does not fail dispatch closed on isolation; a release that retains this criterion remains blocked until a future phase supplies the proof or the owner explicitly revises the release requirement.
2. Real minimum supported Claude/Codex host and CLI versions have fixtures; initialization validates Claude's project per-server timeout, Codex's project-resolved startup/tool timeouts, Claude safe-mode/tool/MCP/slash-command/persistence constraints, Codex config/rules/project-document suppression, and each host's actual project-registration behavior.
...
- [x] A chain proves its own initialization: revision 1 carries the complete `task-initialization` or `legacy-import-initialization` artifact it names, the validator subject admits that artifact alongside the chain, and a chain naming an initialization it cannot supply fails with a stable error.
- [x] Every accepted import begins at revision 1 or names an authenticated predecessor target. The expected-state-revision field is present exactly when existing state is supplied and absent otherwise; state whose revision matches the predecessor but whose digest does not fails, and so does state whose `adopted_checkpoint` is absent or names a different checkpoint than the wrapper's predecessor.
- [x] Every checkpoint's task and repository identity equals the wrapper's, and so does the embedded initialization's own task and repository identity. A digest-valid chain assembled from checkpoints of two tasks fails as an isolation violation instead of validating, and so does an import embedding another task's otherwise-valid initialization.
- [x] Predecessor gaps, self-digest mismatches, reordered or duplicated revisions, and unknown fields fail with stable errors, and the greatest valid chain is identified from an unordered candidate set without consulting server state, with a fork or collision reported as an explicit stop rather than resolved by preference.
- [x] `manual-checkpoint` and `manual-checkpoint-import` pass the same Ajv/Zod agreement harness as Phase 7's union-reachable shapes.
- [x] Chain invariants live in Phase 7's consolidated semantic validator, extended in place; no second semantic authority exists.

**Implemented 2026-07-28.** Phase 8 shipped the two durable roots, their Zod mirrors, a 44-literal consolidated semantic ladder, exact state-to-predecessor checkpoint provenance, and a pure candidate-set selector. Implementation tightened one interface beyond the design: continuation revisions use a parser-proven `ContinuationCheckpointRevision` subtype because the pinned `SafeInteger` seam admitted revision 1 and could not satisfy its own compile-time negative. The selector also enforces the embedded initialization's task and repository identity as a whole-set `foreign-candidate` stop, aligning the pure selection path with validator ranks 5j/5k. No handler, reader, writer, adoption, or reconciliation path is added here.
...
- [x] Current state authenticates the exact immutable receipt and result identity returned by replay; an unreferenced prepared receipt is never success, receipt/result substitution fails, and no post-commit receipt rewrite is required.
- [x] Closed per-operation request constructors admit only the operation's semantic fields. Request digests exclude `intent_id`, `expected_revision`, timestamps, attempt/retry/transport data, and any alternate spelling cannot smuggle excluded state through an open object.
- [x] CAS is checked on every invocation, including replay, before intent handling. Changed intent reuse returns `INTENT_MISMATCH`, stale writers return `STATE_CONFLICT`, and two same-filesystem processes yield one winner while independent task locks do not contend.
...
- [x] Exact config bytes are checked against the state pin before transition work; any byte change returns `PINNED_CONFIG_MISMATCH` without echoing content. The design claims atomic reader visibility, not guaranteed survival of the newest rename across power loss.
- [x] Focused contract, unit, race, and crash tests pass on Node `24.15.0` and `24.18.0`; dependency policy/notices accept the exact permissive pins. The inherited three release-integrity failures remain separately identified unless their existing contradiction is deliberately resolved.

**Implemented 2026-07-29.** Phase 9 shipped the schema-only immutable intent receipt, complete committed-state binding, closed all-tool request digests, deep-frozen parsed calls, live repository/task authority, canonical readers, temp+link exclusive receipt installation, atomic state replacement, a non-renewing `mkdir` task lock, and the mature-state transaction kernel. State is the sole commit authority; matching receipt-only crashes resume without preparation, committed replay requires refreshed CAS, and ambiguity is resolved only from durable rereads. The final counter-review closed exact predecessor revision agreement, expanded all 23 receipt semantic checks and five selector goldens, and added real child-side `SIGKILL` coverage at all four transaction cut points. No initialization, reconciliation, payload projection, or MCP handler was added.

The full suite passed 1,163 of 1,166 tests. The three failures are exactly the inherited `release-offline` contradiction already recorded by Phases 6–8; every Phase 9-owned contract, unit, race, crash, dependency, notice, boundary, and temporary-bundle check passed. The complete decisions, deviations, and verification record are in `phases/phase-9-transaction-substrate-and-exact-replay-log.md`.
...
**Requirements**: REQ-26, REQ-29, REQ-30, REQ-31, REQ-32 (counter-review only; isolation portion unmet), REQ-34, REQ-35

**Scope**: Implement immutable host identity, routing/family validation, a versioned bounded counter-review envelope, exact Claude/Codex adapters, version and logged-in-bit preflights under the scrubbed child environment, generated clean homes with the one credential-file link, environment scrubbing, planted-canary output scans, bounded child lifecycle/failures, serialized dispatch, and server-attested review minting through a non-public capability minter. Return conservative managed-policy telemetry without gating so Phase 15 can persist it with the attempt. Add no sandbox provider, probe gate, new dependency, new error code, or claim of enforced repository-read/process containment; Phase 14 owns adjudication envelopes.

**Success Criteria**:

- [x] Real handshake fixtures identify supported Claude Code/Codex versions; missing/ambiguous/unsupported identity and same-family routing launch no child.
- [x] Both producer directions dispatch a fresh PID/session and expected family using the pinned suppression flags, generated home, scrubbed environment, and non-repository temp cwd. Provider-key/routing sentinels do not reach the child, and planted canaries are absent from output/diagnostics; a leaking negative control proves the substring check can fail.
- [x] Both first-party CLIs report logged in under the exact generated-home/scrubbed child environment without ArchFlow reading, copying, injecting, prompting for, persisting, or logging credential values. The generated home contains only generated defaults and the single credential-file link; API-key/provider-routing variables remain scrubbed.
- [x] The canonical counter-review envelope is versioned, size-bounded, hashed, and structurally unable to carry upstream documents, constitution, producer/triage/prior-review context, repository content, or agent instructions. This criterion does not claim the child lacks independent filesystem/tool access.
- [x] Missing CLI, logged-out auth, bad/unsupported model, rate limit, timeout, cancellation, output overflow, invalid JSON, nonzero exit, and I/O failure remain distinguishable; termination is attempted on stop paths and none produces pass evidence. Escaped-descendant death is not claimed.
- [x] `node:child_process` is used with no new dependency; the rejected sandbox/process-tree candidates and corrected aggregation/license analysis are recorded without adoption.
- [x] Architecture and PRD traceability state explicitly that Phase 13 does not supply OS containment, a probe gate, a tool-surface proof, full REQ-32, or VAL-07; the owner's accepted best-effort limitation does not silently pass the future release sandbox criterion.
- [x] The Anthropic subscription-authenticated dispatch path remains release-disabled until written clarification or a qualified legal determination satisfies `VAL-14`; implementation or fixture success is not legal approval.
...
**Scope**: Implement direct counter-review/adjudication services, structured result validation, the pre-commit verified-render/post-commit authority split, durable self-review/triage/adjudication retention, fixed-point currency and re-entry decisions, phase-instance attempt bounds, guarded retry-effect gate enactment with a handler-supplied fingerprint seam, explicit approved-policy-base constitution resolution, fail-closed `review_trigger`/`enforced_by` handling, separate compliance/drift verdicts, unresolved task-local constitution-edit rejection, and upstream evidence invalidation/reopening. Live MCP/helper assembly remains Phase 15.

**Success Criteria**:

- [x] Accepted findings across two rewrites invalidate stale evidence, rerun self/counter/triage, and adjudicate only the final digest; blockers never authorize advancement and exhaustion gates.
- [x] The deterministic fake-CLI corpus distinguishes seeded substantive defects from clean controls. Real-host quality thresholds are measured and explicitly approved in Phase 21 rather than inferred here.
- [x] Positive, negative, and uncertain triggers plus current/missing/stale/failed mechanical evidence behave fail-closed against the approved policy-base constitution.
- [x] Aligned and incidental drift are recorded correctly; material drift gates and an accepted change reopens the affected upstream approval/evidence chain.
...
**Scope**: Verify the Phase 15 tracked bundle/manifest, then extend `install.sh` for both bundle launchers and skill targets, replacing hard-coded `STALE_SKILLS` deletion with an ArchFlow-owned install manifest that removes only obsolete owned files and preserves unrelated skills; create `archflow-init`; scaffold assets including the default opposite-family model/effort routing in the generated `config.yaml` template; require `claude mcp add --scope project` to create/update committed shared `.mcp.json` with the PATH command `archflow-mcp`, then authenticate and set its persistent one-hour timeout from the parsed project file; write a marker-delimited project Codex block directly because `codex mcp add` has no project scope and cannot express the one-hour tool timeout; preserve unrelated configuration bytes; detect collisions, untrusted configuration, unsupported versions/auth, and missing launcher capability; report managed-context and best-effort-isolation limitations; and add recovery guidance.

**Success Criteria**:

- [x] Installation from a clean checkout starts the exact pinned bundle offline on every claimed Node/host environment.
- [x] Init reruns byte-stably, preserves unrelated JSON/TOML/settings, and reports collisions, malformed/untrusted config, unsupported runtime/CLI/auth, managed-context telemetry, and the absence of enforced sandbox proof without falsely claiming isolation.
- [x] Claude registration uses `claude mcp add --scope project archflow -- archflow-mcp`, produces the PRD-required shared/committed `.mcp.json`, contains no absolute home/install path, preserves unrelated project server content, and records `timeout: 3600000` in the project entry even when a higher-precedence same-name entry masks runtime diagnostics. Codex registration owns only its marker-delimited project block, calls `codex mcp add` nowhere, and preserves every unrelated config byte.
- [x] The project `.mcp.json` carries `timeout: 3600000`; a trusted project's `codex mcp get archflow --json` resolves `startup_timeout_sec: 30` and `tool_timeout_sec: 3600`. Pending Claude approval and untrusted Codex project state are reported as human boundaries, never fixed or claimed as success by init; no `MCP_TOOL_TIMEOUT` export/inheritance branch exists.
- [x] The owned install manifest upgrades/removes only ArchFlow-owned obsolete files; unrelated Claude/Codex skills and host files remain byte-identical.
- [x] Initialized state is Git-trackable/shareable, contains no machine-specific executable path in portable task state, and works after repository relocation and in linked/space/Unicode worktrees.
- [x] `archflow-init` creates no task state and no commit. Fresh task creation refuses to start until workflow/constitution assets resolve from an explicitly approved immutable commit and then stages the normal initialization manifest.
...
**Scope**: Rewrite the five phase skills and status as normal-mode marshallers over the fixed pipeline; preserve one source for both clients; use implementation manifests; enforce workflow approval, exact-diff commit authorization, and post-authorization observation of the committed target-ref/HEAD tree before advance or completion; update canonical parents/notes; offer deterministic gate-ID supplemental prompts; direct the second-terminal human to ingest the other client's structured JSON through `archflow-local`; and reconcile before truthful status. Manual fallbacks and upgrade are later phases.

**Implemented 2026-08-03.** The five phase skills and status now marshal the normal durable pipeline through the existing five MCP tools and the offline helper. Produced-artifact authority is the retained produce manifest digest, phase designs receive explicit artifact approval before implementation, status reconstructs complete superseding gate interfaces from durable requests, and advancement/completion requires immutable commit ancestry and tree proof rather than authorization alone. The tracked MCP/local release was rebuilt and reproduced after explicit user re-acceptance of the unchanged local-only `fast-uri@3.1.0` exposure for MCP bundle `bd36e2fdcfd2dc2609f75cd74166842ddfffcc8c7a170182e417e112bb4a8ab1`.

**Success Criteria**:

- [x] No skill branches on host identity or adds workflow tools; the same source installs and drives both producer directions.
- [x] Phase design cannot become `DESIGNED` or permit code until its retained artifact has explicit `artifact-approval`; phase implementation authenticates that approved artifact as an upstream, cannot stage or commit before explicit final-digest authorization, and cannot advance or complete until the approved current target's committed tree matches every retained output.
- [x] Every human gate offers the optional other-client prompt; the other client emits structured JSON and, from a second terminal, the human runs `archflow-local` in normal/manual mode to validate, bind, canonically render, and atomically rename it to the gate-counter path. Decline records no fabricated review.
- [x] The server sees only complete supplemental projections through a poll-only 500 ms loop. The gate-pending transition table admits only supplemental ingestion, triage, explicit decline/decision, cancellation, or supersession; `SUPPLEMENTAL_REVIEW_REQUIRED` rejection resumes the gate and accepted change supersedes it and re-enters the fixed point.
- [x] Status verifies the immutable whole-config digest before recommending work; `PINNED_CONFIG_MISMATCH` reports no config content and recommends only a distinct new task or explicit upgrade for intentional routing/model/effort changes.
...
- [x] Normal and manual journeys prove no restore collision overwrites silently: discard-and-restore touches only declared paths, adopt requires changed declared inputs/fingerprint plus rationale and creates a new generation, and abort leaves state/worktree non-advancing.
...
- [x] A real task reaches the snapshot boundaries, receives `SNAPSHOT_LIMIT` above either cap without partial authority, retains all current/evidence chains, records a safe maintenance prune, and rejects a secret-bearing checkpoint before Git projection.
