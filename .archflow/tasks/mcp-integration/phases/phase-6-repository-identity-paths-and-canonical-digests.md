...
**Read this before using the list below.** This inventory has been found incomplete three times across successive reviews — and a **fourth** time by the mandated mechanical sweep at implementation time, which is exactly why the sweep is mandated. **It is a cross-check, not an authority.**

**The fourth-round delta, recorded so it is durable.** Sites the prose list omitted, all real and all retightened: `review.ts`, `triage.ts`, and `adjudication.ts` plus their five schemas; `errors.ts` `task_id` (`taskPathClass`, `TASK_INVALID`) and `gate_id` at five further codes; `authority-link.schema.json`; four more `trust.ts` `task_id` interface fields; and two `mcp-tools.schema.json` `task_id` sites beyond the six enumerated (`waiverOrigin.task_id` and the waiver-success `task_id`). Sites the prose list named that **do not exist**: `input_id` has zero occurrences anywhere in the repository, and `prior_gate_id` does not appear in `mcp-tools.ts`. **One consequence needs a human's eye:** at `review.ts`/`triage.ts`/`adjudication.ts` the pre-existing `task_id` used a *local kebab-only* pattern (`^[a-z][a-z0-9]*(-[a-z0-9]+)*$`), not `safeId`, so retightening there is a **vocabulary swap rather than a pure narrowing** — it caps length at 64 but newly admits `.`, `_`, and a leading digit. The implementing chunk must **regenerate it mechanically** — grep every occurrence of each logical ID name across all Zod modules in `src/contracts/`, every exported TypeScript interface, all 24 normative JSON Schemas, the advertised-schema closure, and every fixture under `test/` — then reconcile the generated set against the prose list. **Any difference is a design defect to report back, not something to silently absorb**, in either direction: a site the list omits, and a site the list names that no longer exists.
...
**Containment is seven steps, in this exact order**: (1) reject NUL, `path.isAbsolute(input)`, or a win32 drive letter *before* resolving; (2) `candidate = path.resolve(root, input)`; (3) `realRoot = await realpath(root)`, using the same nearest-existing-ancestor walk as step 4, because the task root legitimately does not exist before task initialization; (4) `realCand = await realpath(candidate)`, and on `ENOENT` realpath the nearest existing ancestor and re-append the missing tail; (5) `rel = path.relative(realRoot, realCand)`; (6) accept only if `rel === "" || (rel !== ".." && !rel.startsWith(".." + path.sep) && !path.isAbsolute(rel))`; (7) on the real open, OR in `(fs.constants.O_NOFOLLOW ?? 0)`.

**Step 6's `rel !== ".."` guard is load-bearing and was missing from the first three revisions of this design.** `"..".startsWith("../")` is `false`, so the condition as originally written *accepted the root's own parent* — the exact escape the check exists to prevent. `release-support.mjs`'s private `isInside` carries the guard explicitly; this document had dropped it. Corrected during implementation after the phase-6 implementation counter-review, with a dedicated regression test.
...
| Modify | `src/contracts/mcp-tools.ts` + `mcp-tools.schema.json`; `gates.ts` + `gate-decision.schema.json`; `trust.ts`; `supplemental.ts` + `supplemental-review.schema.json`; `errors.ts` + `project-error.schema.json`; `result-expectation.schema.json`; `evidence-slots.schema.json`; **plus the sites the mechanical sweep discovered:** `review.ts` + `review.schema.json` + `review-evidence.schema.json`; `triage.ts` + `triage.schema.json`; `adjudication.ts` + `adjudication.schema.json` + `adjudication-evidence.schema.json`; `authority-link.schema.json` | 3 | The mechanically regenerated ID inventory, applied atomically across Zod and JSON Schema. |
...
- **Git runner discipline** follows the established `scripts/release-support.mjs` style, now in TypeScript: `execFile` with an argv array and no shell, explicit `cwd`, `encoding: "buffer"` where bytes matter, explicit `maxBuffer` and `timeoutMs`. Every call site names its own argv; there is no dynamic command construction, and the `:(top,literal)` prefix is mandatory wherever a pathspec appears. `--literal-pathspecs` is **forbidden**: it disables pathspec magic, so the `:(top,literal)` prefix would then be matched as a literal filename and select nothing (reproduced on git 2.43 — empty output), and placed after the subcommand `ls-files` and `check-attr` reject it outright. `check-attr` takes pathnames rather than pathspecs, so it needs neither.
...
- [ ] Every reader below discovery accepts only a `RootBoundGitRunner`; holding an unrooted runner after discovery is a type error. All pathspecs use the `:(top,literal)` prefix alone (never `--literal-pathspecs`, which would disable that magic), `check-attr` receives literal pathnames, and every returned path is asserted byte-equal to the requested claim.
