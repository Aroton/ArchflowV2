...
**Goal**: Consume Phase 3's authenticated seam to implement and prove the complete inert low-level beta.5 stdio runtime.
...
The target is protocol `2025-11-25` with exact `@modelcontextprotocol/server@2.0.0-beta.5` and locked core beta.5. Both were current and MIT on 2026-07-27, but v2 remains beta. Live dist tags, official guidance, and every required public API are checked before the first implementation mutation and again immediately before final verification/review. Any tag, stable-status, guidance, or public-surface drift stops for design/architecture review without package mutation; no continuous polling is required. Low-level `Server` is a documented advanced-use exception and `src/mcp/sdk-adapter.ts` is the sole production SDK import owner. Raw framing, session policy, output queueing, and process ownership are separate SDK-free modules using Node streams only; there is no SDK stdio/framing owner, private import/patch, or new framing dependency.
...
| Modify | `package.json`, `.github/workflows/ci.yml` | Add focused probe/runtime/policy commands and exact Node jobs without dependencies. |

No dependency/lock, contract schema, catalogue, tracked `dist`, persistence, dispatch, gate, local helper, installer, or release bundle change belongs here.

## Work Breakdown

1. **Compatibility and currency gate**: Check live currency before mutation; prove exact public construction, malformed/retry mutation boundary, handler/projection behavior (including beta.5 `TOOL_DISABLED` rewrite/restoration), custom facade, and teardown on both Node patches. Stop on drift or failed invariant.
2. **Boundary repair**: Move known argument classification before handler availability in `server.ts` and prove unknown, inert, enabled, portable/runtime semantic, and at-most-once behavior.
3. **Bounded framing**: Implement/test strict UTF-8 JSONL, CRLF/empty/partial EOF, 10 MiB caps, retained current-chunk remainder, and fatal parse/overflow behavior.
4. **Session and routing**: Implement/test closed route table, exact top-level and list/call params allowlists, SDK metadata/task preservation, initialization transaction state, lifetime ledger/caps, monotonic IDs, active/cancelling/queued records, same-chunk order, and authentic expected outcomes.
5. **Send queue**: Implement/test universal FIFO entries, 10 MiB/1,024 caps, admission versus callback completion, false/drain flow control, fatal callbacks, association, and close cleanup.
6. **SDK adapter**: Implement exact Server/capability/handler/connect order, context minting, missing-argument pass-through, present-non-object surrogate recovery, SDK `_meta`/task validation, three-domain projection, retained-authentic canonicalization, and terminal mutation failures.
7. **Process/runtime assembly**: Implement runner/main/termination seam, once-only diagnostics/exits, shared temporary builder, and self-contained integration harness.
8. **Live/adversarial verification**: Complete all transcripts/races/caps, exact Node verification, second currency check immediately before final review, and aggregate checks.

Eight bounded delegated chunks fit the implementation budget because each owns a distinct module/test seam; an immediate architecture phase split is unnecessary.

## Success Criteria

- [ ] Currency/public API checks pass before mutation and immediately before final verification; any drift or public-hook failure stops without package mutation/private workaround.
...
- [ ] All four protocol mappings are unit-proven, live routes use retained authentic outcomes, beta.5 `TOOL_DISABLED` rewriting is restored, and SDK/project/direct/fallback frames remain byte-stable with safe `-32603` containment.
- [ ] Universal bounded FIFO/framer flow control pauses/resumes input correctly and leaves no queued entry, record/tombstone, listener, pending completion, post-close adapter effect, or caller-stream destruction.
...
- [ ] Focused/full/live/policy/dependency/notices/aggregate checks pass under exact Node `24.15.0` and `24.18.0` with no new dependency, schema, tracked release, persistence, dispatch, or REQ-33 completion claim.

## Verification Steps

1. Before mutation and again immediately before final verification/review, use live official/npm sources to verify beta.5/core tags, status/guidance, MIT roots, protocol support, and required public declarations. Stop on any drift.
