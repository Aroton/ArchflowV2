...
Use `archflow-local upgrade --task <task>` with a complete plain-JSON payload naming `source_root`, `task_id`, `policy_base_commit`, `import_baseline_commit`, `code_baseline_commit`, and any `exclude` paths. Pipe that payload as JSON directly on stdin (for example `printf '%s' '<json>' | archflow-local upgrade --task <task>`); never write it to a scratch file — `--input <json-file>` remains supported but is unnecessary for generated input. Inspect the result's `ok` field rather than relying on process exit status. Preserve its initialization manifest, audit context, manifest path, resume phase, staged paths, unmapped paths, and draft sources exactly; do not calculate or edit their digests.
