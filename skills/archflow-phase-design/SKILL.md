...
For `self_review`, apply every rubric criterion and pinned rule to the current artifact. Create the agent-declared review only from the current subject/fingerprint, the effective self-review route returned by status, and the locally hashed rubric. Install it through `archflow_state`; to preview its canonical projection, pipe `{"kind":"review","value":<that review artifact>}` to `archflow-local render` on stdin (`archflow-local --help` lists each command's payload).
