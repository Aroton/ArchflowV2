...
For `self_review`, apply every stable-rubric criterion and pinned rule to the current design. Build the agent-declared review from the current subject/fingerprint, status-provided effective self-review route, and locally hashed rubric. Record it as a review-evidence artifact with `archflow_state`; to preview its canonical projection, pipe `{"kind":"review","value":<that review artifact>}` to `archflow-local render` on stdin (`archflow-local --help` lists each command's payload).
