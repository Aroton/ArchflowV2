...
Understand the ask and inspect relevant code and repository instructions. Record the initial full HEAD commit as `base_commit` and note pre-existing edits. Preserve those edits; explain when a declared output already contains unrelated work. Produce a concise, self-contained plan: the intended outcome and scope, the approach grounded in current code, any consequential choices, and useful verification. Settle choices that could materially change what the user gets; leave routine coding choices to implementation. Size the explanation to the ask. This plan needs neither a PRD nor architecture/phase documents, and ordinary work does not need a decomposition or a prescribed template.

Call `archflow_review` with `schema_version:"1"`, `stage:"plan"`, the original `ask`, the plan text, `base_commit`, and repository-relative file `paths` the plan expects to change, plus optional `review_routes`. Paths may name planned new files; use file names rather than directories. The MCP uses the standalone plan rubric for counter-review, with test-strategy review and constitution review when active rules exist, and no effort review. It returns reports, constitution judgments, human-review reasons, and policy digests.
