...
Submit the finished bytes through the current offer with `{"kind":"work-result","outcome":"succeeded"}` (or the offered failed form with a concrete reason). Do not supply paths, policy, routing, digests, revisions, or other server-owned facts. Apply the next no-submission `review` offer. The server selects the independent configured rubric and constitution review (opposite-family by default) from the returned `review_context`; never perform, spawn, simulate, or replace that review. If you run any same-side author review, use `review_context.rubric` verbatim and the returned active rules, never an authored rubric.
