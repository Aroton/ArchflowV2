...
1. Draft `.archflow/tasks/<task>/prd.md`, then run `archflow-local build-document --task <task>` for `phase_instance: "prd"`, `step: "produce"`, `document_path: "prd.md"`, and `declared_inputs: [{"input_id":"user-ask","path":".archflow/tasks/<task>/ask.md"}]`, initially using the fingerprint reported by status. The helper reads `ask.md` itself and records its digest; a missing `ask.md` fails the build rather than producing an undeclared PRD.
