...
describe("Phase 20 secret rejection propagation", () => {
  ...
  it("a cut immediately before scan cannot leave a partial projection", async () => {
    ...
    const before = durableShape(h);
    const cut: SecretScanner = { scan: async () => { throw new Error("phase20-cut-before-scan"); } };
    await expect(prepareProjectionPlan([source], cut, h.root as ResolvedTaskPath)).rejects.toThrow("phase20-cut-before-scan");
