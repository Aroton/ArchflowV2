...
import { importGateDecisions, runDurableGate } from "../../src/state/gates.js";
...
const state = (): TaskStateV1 => ({ schema_version: "1", task_id: parseTaskSlug("task-1"), repository_identity_digest: D("1"), revision: parseSafeInteger(4), phase_instance: "phase-impl-2" as TaskStateV1["phase_instance"], step: "produce", status: "running", attempt: parseSafeInteger(1), input_fingerprint: D("2"), initialization_digest: D("3"), config_digest: D("4"), workflow_digest: D("5"), constitution_digest: D("6"), policy_base_commit: "abcdef0123456789abcdef0123456789abcdef01" as TaskStateV1["policy_base_commit"], authoritative_results: [], approvals: [], waivers: [] });
...
describe("gate manual authority import", () => {
  ...
  it("leaves an aborted wait pending and resumes through archive, receipt, state, and cleanup", async () => {
    ...
    const lifecycle = { authority, expected_revision: initial.revision, intent_id: "intent-1" as never, request_digest: D("b"), input_fingerprint: inputFingerprint, phase_instance: initial.phase_instance, summary: "Approve", subject_digest: D("c"), current_evidence: { set_digest: D("3"), slots: [self, counter] as const }, kind: "artifact-approval" as const, context };
    ...
    const closure = decision("approve"); if (closure.outcome !== "decided") throw new Error("fixture decision mismatch");
    writeFileSync(join(taskRoot, "gate.decision"), canonicalDocument({ ...closure.envelope, gate_id: lifecycleGate }).bytes);
