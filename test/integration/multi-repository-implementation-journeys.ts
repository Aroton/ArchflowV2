...
export function registerMultiRepositoryImplementationJourney(selected: string): void {
  describe("multi-repository implementation journeys", { timeout: TIMEOUT }, () => {
    ...
    register("completes primary then api under no-wait authority and retains prior proof across a descendant", async () => {
    ...
    register("opens the human commit gate on a content rule matched only in the api repository and then covers both repositories' commit facts", async () => {
    ...
    register("stops after the primary proof when the uncommitted api target moves", async () => {
    ...
    register("returns a named blocked status when the api target moves before its commit gate opens", async () => {
    ...
    register("settles a granted policy exception under a content-rule wait without observing secondary commits", async () => {
    ...
    register("qualifies api-only adopt, restore, worktree deletion, and committed deletion with a same-path primary", async () => {
