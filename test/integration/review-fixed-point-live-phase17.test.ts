...
const task = parseTaskSlug("phase17-live-fixed-point");
...
async function fixture() {
  const root = realpathSync(await mkdtemp(join(tmpdir(), "archflow-phase17-live-")));
  roots.push(root);
  ...
  const bootstrap = await createProductionServices({
    ...
    task_id: task,
    operation: parseSafeCode("phase17-live-bootstrap"),
  ...
  const services = await createProductionServices({
    working_directory: root,
    task_id: task,
    operation: parseSafeCode("phase17-live-resolve"),
  });
...
describe("Phase 17 live fixed-point regressions", () => {
  it("installs successive implementation outputs through the real state handler", async () => {
    ...
    const connection = connectionContextFactory.captureStartup({
      connection_id: "phase17-implementation-produce",
    ...
    execFileSync("git", ["commit", "-qm", "phase 17 implementation"], { cwd: h.root, env: environment });
    ...
    execFileSync("git", ["add", "--", outputPath], { cwd: h.root, env: environment });
    execFileSync("git", ["commit", "-qm", "phase 18 implementation"], { cwd: h.root, env: environment });
    completion = await createProductionServices({
  ...
  it("closes the real handler pipeline on the retained produce artifact digest", async () => {
    ...
    const scanner = {
      scan: async (candidates: readonly { virtual_path: ReturnType<typeof parseRepositoryPathClaim> }[]) => ({
        schema_version: "1" as const, outcome: "clean" as const, detector_set_id: parseSafeId("phase17-upstream-test"),
...
if (argv.length === 1 && argv[0] === "--version") process.stdout.write("codex-cli 0.146.0\\n");
  ...
  process.stdout.write('{"type":"turn.completed"}\\n');
    ...
    const connection = connectionContextFactory.captureStartup({
      connection_id: "phase17-pipeline", startup_repository_candidate: { working_directory: h.root },
