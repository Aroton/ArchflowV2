...
beforeAll(async () => {
  bundleRoot = await mkdtemp(resolve(tmpdir(), "archflow-local-phase18-"));
...
async function repository(): Promise<string> {
  const root = mkdtempSync(join(tmpdir(), "archflow-phase18-manual-repo-"));
  roots.push(root);
...
async function initializeNormalState(root: string): Promise<void> {
  ...
  const services = await createProductionServices({
    ...
    task_id: taskId,
    operation: parseSafeCode("phase18-normal-bootstrap"),
  });
  ...
  const initialInput = {
    ...
    task_id: taskId,
    intent_id: "phase18-normal-initialize",
    expected_revision: 0,
...
async function committedImplementationState(root: string, plannedFinalPhase: number): Promise<{
  ...
  const produced = await invokeState(root, {
    schema_version: "1", task_id: task, intent_id: `phase18-implementation-${plannedFinalPhase}`,
  ...
  }, `phase18-implementation-${plannedFinalPhase}`);
  ...
  const gateId = `phase18-commit-${plannedFinalPhase}`;
  const context = {
  ...
  const request = parseGateRequest({
    schema_version: "1", gate_id: gateId, intent_id: `phase18-commit-intent-${plannedFinalPhase}`,
  ...
  const decision = parseGateDecisionRecord({
    ...
    supplemental: [], outcome: "decided", envelope: {
      ...
      human_provenance: { schema_version: "1", actor_class: "human", assurance: "declared-local-trace",
        channel: "archflow-local", decision_event_id: `phase18-commit-decision-${plannedFinalPhase}`,
        helper_invocation_id: `phase18-commit-helper-${plannedFinalPhase}`, recorded_at: "2026-08-03T12:00:00.000Z" },
...
describe("Phase 18 bundled manual workflow", () => {
  ...
  it("stops a divergent manual handoff without changing either head or fabricating a record", async () => {
    const parent = mkdtempSync(join(tmpdir(), "archflow-phase18-divergent-handoff-"));
    roots.push(parent);
  ...
  it("builds all three import wrappers and adopts both mature modes through the real handler", async () => {
    ...
    const initialImport = cli(initialRoot, "manual-next", {
      schema_version: "1", operation: "import-call", intent_id: "phase18-import-initial",
    });
    ...
    const initialHandled = await invokeState(initialRoot, initialImport.value.value.call.input, "phase18-initial-handler");
    ...
    const stateAnchoredImport = cli(matureRoot, "manual-next", {
      schema_version: "1", operation: "import-call", intent_id: "phase18-import-state-anchor",
    });
    ...
    const anchoredHandled = await invokeState(
      ...
      "phase18-state-anchored-handler",
    );
    ...
    const continuationImport = cli(matureRoot, "manual-next", {
      schema_version: "1", operation: "import-call", intent_id: "phase18-import-continuation",
    });
    ...
    const rejected = await invokeState(matureRoot, {
      ...continuationImport.value.value.call.input,
      intent_id: "phase18-import-stale-predecessor",
      expected_revision: 1,
    }, "phase18-rejected-mature-handler");
  ...
  it("derives gate and waiver archives from selectors before checkpoint import", async () => {
    ...
    expect(cli(root, "manual-next", {
      ...
      result: { result_id: "phase18-gate-produce", artifact: built.value.value },
    })).toMatchObject({ status: 0, value: { ok: true } });
    ...
    const review = (role: "self-review" | "counter-review") => ({
      ...
      rubric_digest: canonicalJsonDigest({ schema_version: "1", rubric: "phase18" }),
    ...
    for (const role of ["self-review", "counter-review"] as const) {
      ...
      const installed = cli(root, "manual-next", {
        ...
        result: { result_id: `phase18-${role}`, evidence: { kind: "review", evidence: review(role) } },
      });
    ...
    const provenance = (suffix: string) => ({
      ...
      decision_event_id: `phase18-${suffix}-decision`, helper_invocation_id: `phase18-${suffix}-helper`,
    ...
    const importCall = cli(root, "manual-next", {
      schema_version: "1", operation: "import-call", intent_id: "phase18-gate-waiver-import",
    });
    ...
    const imported = await invokeState(root, importCall.value.value.call.input, "phase18-gate-waiver-handler");
  ...
  it("imports authenticated committed implementation successors and final completion through the real handler", async () => {
    ...
    const completedImport = cli(finalRoot, "manual-next", {
      schema_version: "1", operation: "import-call", intent_id: "phase18-final-commit-import",
    });
    ...
    const completed = await invokeState(finalRoot, completedImport.value.value.call.input, "phase18-final-commit-handler");
    ...
    const successorImport = cli(successorRoot, "manual-next", {
      schema_version: "1", operation: "import-call", intent_id: "phase18-nonfinal-commit-import",
    });
    expect(successorImport, JSON.stringify(successorImport)).toMatchObject({ status: 0, value: { ok: true } });
    const advanced = await invokeState(successorRoot, successorImport.value.value.call.input, "phase18-nonfinal-commit-handler");
