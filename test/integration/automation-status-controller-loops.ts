...
export function registerAutomationStatusControllerLoop(selected: string): void {
  describe("automation status controller loop", { timeout: TIMEOUT }, () => {
    ...
    register("reaches complete by launching and resuming only returned descriptors", async () => {
...
`);
    ...
    register("keeps a configured SQL implementation human-owned until the interactive producer resolves it", async () => {
      const workspace = await createTaskWorkspace({
...
`);
    ...
    register("projects configured PRD and design approvals through the same token-free human boundary", async () => {
      const workspace = await createTaskWorkspace({
    ...
    register("gives an exact-current invocation-route dispatch failure precedence over review continuation", async () => {
      const workspace = await createTaskWorkspace({
    ...
    register("projects a constitution-triggered document decision as an exceptional human boundary", async () => {
      const workspace = await createTaskWorkspace({
