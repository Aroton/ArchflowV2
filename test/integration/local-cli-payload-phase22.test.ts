...
const roots: string[] = [];
const task = parseTaskSlug("phase22-payload");
...
beforeAll(async () => {
  bundleRoot = await mkdtemp(resolve(tmpdir(), "archflow-local-phase22-"));
...
function cliFile(root: string, command: string, raw: string): Readonly<{ status: number | null; stdout: string; stderr: string; path: string; value?: any }> {
  const payloadRoot = mkdtempSync(join(tmpdir(), "archflow-phase22-payload-"));
...
async function repository() {
  const root = mkdtempSync(join(tmpdir(), "archflow-phase22-repo-"));
  roots.push(root);
...
describe("Phase 22 payload input modes and error taxonomy", () => {
