...
const task = parseTaskSlug("phase19-upgraded");
const roots: string[] = [];
let bundleRoot = "";
let localBundle = "";
...
beforeAll(async () => {
  bundleRoot = await mkdtemp(resolve(tmpdir(), "archflow-local-phase19-"));
...
async function repository(): Promise<{ root: string; source: string; head: string }> {
  const root = mkdtempSync(join(tmpdir(), "archflow-phase19-upgrade-repo-"));
  roots.push(root);
...
async function initializationCall(root: string, initialization: ReturnType<typeof parseLegacyImportInitialization>, intentId: string) {
  ...
  const draft = {
    ...
    input_fingerprint: "0".repeat(64), phase_instance: "prd" as const, step: "produce" as const,
...
describe("Phase 19 bundled legacy upgrade workflow", () => {
  ...
  it("opens a real migration audit only over approved design authority and authenticates the resume jump", async () => {
    ...
    const scanner = { scan: async (candidates: readonly { virtual_path: string }[]) => ({
      schema_version: "1" as const, outcome: "clean" as const, detector_set_id: parseSafeId("phase19-integration"),
    ...
    if (!produceEnvelope.ok) throw new Error(JSON.stringify(produceEnvelope));
    const produced = await invokeState(root, {
      ...produceDraft,
      input_fingerprint: produceEnvelope.value.input_fingerprint,
      artifact: { ...phaseArtifact.value, input_fingerprint: produceEnvelope.value.input_fingerprint },
    }, "legacy-resume-produce");
    expect(produced).toMatchObject({
      kind: "project-result", result: { ok: true, value: { status: "succeeded" } },
    });
    const afterProduce = await createProductionServices({
      working_directory: root, task_id: task, operation: parseSafeCode("legacy-after-resume-produce"),
    });
