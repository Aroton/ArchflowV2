...
const roots: string[] = [];
const task = parseTaskSlug("phase17-cli");
...
beforeAll(async () => {
  bundleRoot = await mkdtemp(resolve(tmpdir(), "archflow-local-phase17-"));
...
async function repository() {
  const root = mkdtempSync(join(tmpdir(), "archflow-phase17-cli-repo-"));
  roots.push(root);
...
describe("Phase 17 bundled local CLI", () => {
