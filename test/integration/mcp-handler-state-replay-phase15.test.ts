...
`);
  ...
  const state: TaskStateV1 = {
    ...
    initialization_digest: canonicalJsonDigest({ fixture: "phase-15-handler" }),
    config_digest: configDigest,
    workflow_digest: workflowDigest,
    constitution_digest: constitution.value.digest,
  ...
  const documentBytes = new TextEncoder().encode("Phase 15 implementation notes\n");
...
describe("Phase 15 state handler durable integration", () => {
