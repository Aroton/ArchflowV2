...
/**
 * The sweep. Every array-shaped subschema in the seven schemas must carry an ordering keyword, and
 * the set of them must be exactly the thirteen sets above — nothing exempt, nothing untested.
 */
...
const collectArraySubschemas = (): readonly { readonly location: string; readonly keywords: readonly string[] }[] => {
  ...
  const walk = (node: unknown, location: string): void => {
    ...
    if (record.type === "array" || "items" in record || "prefixItems" in record || "minItems" in record) {
      found.push({
        location,
        keywords: ["x-archflow-sorted-unique", "x-archflow-sorted-unique-by"].filter((keyword) => keyword in record),
      });
    }
...
describe("no array in this phase is exempt from set ordering", () => {
  ...
  it("the arrays the schemas declare are exactly the thirteen sets under test", () => {
 ...
 * positive count pins its own minimum at the field and must not `$ref` it. Nine fields do.
 */
