...
describe("Phase 19 upgrade skill contract", () => {
  ...
  it("enumerates upgrade only on the non-workflow surfaces", () => {
    for (const path of ["install.sh", "CLAUDE.md", "AGENTS.md", "README.md", "test/contracts/skill-contract-phase17.test.ts"]) {
    ...
    const phase17 = readFileSync(resolve(root, "test/contracts/skill-contract-phase17.test.ts"), "utf8");
    const normalPhaseSkills = /const normalPhaseSkills = \[([\s\S]*?)\] as const;/u.exec(phase17)?.[1];
