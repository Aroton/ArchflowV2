...
export function renderProduceReviewMaterial(
  ...
  const changes = subject.retained.projection_plan.entries.map((entry) => Object.freeze({
    path: entry.path,
    after: entry.desired.state === "absent"
      ? Object.freeze({ state: "absent" as const })
      : Object.freeze({ state: "present" as const, ...visibleBytes(entry.desired.bytes) }),
    ...(entry.rollback === undefined ? {} : {
      before: entry.rollback.state === "absent"
        ? Object.freeze({ state: "absent" as const })
        : Object.freeze({ state: "present" as const, ...visibleBytes(entry.rollback.bytes) }),
    }),
  }));
