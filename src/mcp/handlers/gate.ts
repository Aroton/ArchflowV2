...
export async function handleGate(
  ...
  return mapHandlerErrors<"archflow_gate">(context.invocation_id, async () => {
    ...
    // The bounded-decision pair is all-or-nothing. Both present: settle in this one call through
    // the validated preview. Both absent: open the gate and wait for the human decision written
    // through the disposable interface. Anything in between is a contract violation; fail closed.
    ...
    if (!bounded && (call.input.preview_digest !== undefined || call.input.decision !== undefined)) {
      return fail(createProjectError("STATE_INVALID", {
        ...
        issue_code: "gate-decision-required",
      }));
    }
