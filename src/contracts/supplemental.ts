...
const gateCounterSlotSchema = z.object({ role: z.literal("gate-counter-review"), evidence_digest: digest, assurance: z.enum(["server-attested", "degraded"]), producer_family: family, reviewer_family: family, independence: z.literal("opposite-family"), gate_id: pathSafeIdV1Schema }).strict().superRefine((slot, context) => { if (slot.producer_family === slot.reviewer_family) context.addIssue({ code: "custom", message: "gate counter-review must be opposite-family" }); });
