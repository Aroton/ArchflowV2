...
import { ADAPTER_IDS, EFFORT_VALUES, MODEL_FAMILIES, routeOverrideRecordSchema, type AdapterId, type ModelFamily, type RouteOverrideRecord, type RuleVersionRef } from "./review.js";
...
const serverSchema = provenanceBase.safeExtend({ assurance: z.literal("server-attested"), adapter: z.enum(ADAPTER_IDS), cli_version: nonBlank, model_family: z.enum(MODEL_FAMILIES), effort: z.enum(EFFORT_VALUES), invocation_id: id, envelope_input_digest: digest, observed_output_digest: digest, result_id: id, route_override: routeOverrideRecordSchema.optional() }).strict();
