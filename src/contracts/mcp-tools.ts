...
export type CounterReviewFeedbackSuccess = { readonly path: RepositoryPathClaim; readonly reports: readonly ReviewReport[]; readonly constitution: CounterReviewConstitutionOutcomeV2; readonly revision: number; readonly request_digest?: Sha256Digest };
export type CounterReviewSuccess = CounterReviewFeedbackSuccess | CounterReviewV3Success | CounterReviewV2Success | LegacyCounterReviewV1Success;
...
export const toolSuccessSchemas = {
  ...
  archflow_counter_review: z.union([z.object({ path: repositoryPathClaimV1Schema, reports: z.array(reviewReportSchema).min(1), constitution: z.union([z.object({ status: z.literal("not-run"), reason: z.literal("no-active-constitution-rules") }).strict(), z.object({ status: z.literal("evaluated"), path: repositoryPathClaimV1Schema, constitution: z.enum(["pass", "fail", "uncertain"]), triggers: z.array(rule) }).strict()]), revision: safeInteger, request_digest: digest.optional() }).strict(), z.object({
