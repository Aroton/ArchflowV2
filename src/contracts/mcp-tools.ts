...
const common = { schema_version: z.literal("1"), task_id: safeId, intent_id: safeId, expected_revision: safeInteger, input_fingerprint: digest } as const;

export interface CommonToolInput { readonly schema_version: "1"; readonly task_id: string; readonly intent_id: string; readonly expected_revision: number; readonly input_fingerprint: Sha256Digest }
...
export type ResultIdentityPayload<K extends ToolName = ToolName> = { readonly [P in K]: Readonly<{ schema_version: "1"; tool: P; task_id: string; intent_id: string; input_fingerprint: Sha256Digest; request_digest: Sha256Digest; result_id: string; resulting_revision: number; success: ToolSuccess<P> }> }[K];
...
const supersedes = z.object({ superseded_gate_id: safeId, accepted_triage_digest: digest, old_subject_digest: digest }).strict();
...
const waiverOrigin = z.object({ origin_gate_id: safeId, origin_decision_digest: digest, origin_context_digest: digest, task_id: safeId, phase_instance: phase, subject_digest: digest, current_evidence_set_digest: digest, rule, scope }).strict();
...
const successSchemas = {
  ...
  archflow_waiver: z.union([z.object({ origin_gate_id: safeId, waiver_gate_id: safeId, task_id: safeId, rule_id: safeId, rule_version: z.number().int().positive().max(Number.MAX_SAFE_INTEGER), subject_digest: digest, current_evidence_set_digest: digest, scope, human_provenance: provenance, granted: z.literal(true), expires: z.literal("task-complete"), notes: text, revision: safeInteger }).strict(), z.object({ origin_gate_id: safeId, waiver_gate_id: safeId, task_id: safeId, rule_id: safeId, rule_version: z.number().int().positive().max(Number.MAX_SAFE_INTEGER), subject_digest: digest, current_evidence_set_digest: digest, scope, human_provenance: provenance, granted: z.literal(false), notes: text, revision: safeInteger }).strict()])
} as const;
