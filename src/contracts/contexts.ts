...
const initializationSchema = z.object({ client: z.object({ name: text, version }).strict(), host: z.enum(["claude", "codex", "unknown"]), protocol_version: version }).strict();
const invocationSchema = z.object({ invocation_id: id, transport_metadata: z.object({ request_id: z.union([text, z.number().int().safe()]), operation: z.literal("tools/call") }).strict() }).strict();
