...
import { REPOSITORY_NAME_PATTERN } from "./config.js";
...
const PROJECT_PARAMETER_SCHEMAS = {
  ...
  WORKFLOW_INVALID: issueParams, CONFIG_INVALID: object({ issue_code: code, issues: validationIssues.optional() }),
  ...
  REPOSITORY_NOT_FOUND: object({ repository_candidate_digest: digest }), REPOSITORY_MISMATCH: digestsParams, REPOSITORY_VIEW_UNAVAILABLE: object({ repository_name: repositoryName }),
  ...
  RATE_LIMITED: object({ ...adapterAttempt, reason: z.literal("session-limit").optional() }), TIMEOUT: object({ ...adapterAttempt, limit_ms: integer, origin: z.literal("cli").optional() }), CANCELLED: object({ source: z.enum(["client", "transport"]), attempt: integer }), MODEL_OUTPUT_INVALID: object({ ...adapterAttempt, issue_code: code }), IO_ERROR: object({ operation: code, attempt: integer }), OUTPUT_OVERFLOW: object({ adapter, byte_count: integer, byte_cap: integer }), PROCESS_FAILED: object({ adapter, exit_class: code }), INTERNAL_ERROR: object({ correlation_id: id }),
