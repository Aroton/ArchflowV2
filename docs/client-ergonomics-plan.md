...
3. **Composition**: new produce-only `build-request` command: intent + (optionally) document facts → the complete resolved terminal `archflow_state` request. No universal intent router — after the other fixes, status already emits executable requests for everything else (YAGNI per CLAUDE.md).
   ...
   *Extended 2026-08-10 (same day, later run)*: the mid-loop sample that motivated the reversal started after task creation, so create-task stayed a four-move hand-assembly (`task-init` → paste artifact into the prefill → `envelope` → call) — the first step every client meets, and the one that drove a client to script workarounds. `build-request` now also composes kind `initialize` (legal only before durable state; refuses with `TRANSITION_INVALID` once state exists), staging the initialization artifact itself, and a contract test pins the invariant that every prefill status emits maps onto a composer kind. The legacy-import path (`archflow-upgrade`) deliberately stays on envelope: its staging requires a human-reviewed descriptor and a review pause before initialization.
