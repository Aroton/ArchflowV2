...
- No universal build-request intent router; no new NextAction codes; no relaxation of any digest check, closed schema, or transition rule; no scratch-file contract (stdin rule stays); no changes to `transitions.ts` legality semantics — templates conform to the law, never the reverse.
