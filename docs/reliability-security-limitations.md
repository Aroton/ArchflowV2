...
**Existing mitigation:** Preflight records discoverable managed-policy paths and persists that telemetry with dispatch attempts. Explicit CLI flags suppress the ordinary project, user, MCP, session, rule, and skill sources that the supported versions expose. Initialization reports the best-effort isolation limitation rather than treating a clean preflight as proof.
