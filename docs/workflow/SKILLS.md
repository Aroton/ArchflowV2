...
**Explored:** 2026-08-23 · **Commit:** `92fa1f6` · **Covers:** `skills/`, `src/init/`, `src/contracts/semantic-workflow.ts`, `src/mcp/handlers/semantic.ts`, `src/state/semantic-*.ts`, `assets/`
...
The plan is machine-readable by contract — consecutive `### Phase N: Name` headings, or an explicit open-ended marker; anything else fails closed. The server derives `planned_final_phase` from those headings. A returned `design-approval` contains the complete document outcome and every constitution finding for human judgment; otherwise an authenticated no-wait result returns the exact milestone commit directly. Both paths use only server-returned commit facts, verify proof through status, and record the phase-design hand-off.
...
After review, the returned action determines the authority path. If `gate-summary` is offered, it opens `commit-authorization` and the human decision remains mandatory. A clean authenticated no-wait result may return commit facts directly. In both cases the client verifies baseline and target, stages exactly the returned paths, inspects the staged diff and exact message, creates the commit, and calls read-only status for proof. `requires_human_confirmation:true` retains the separate conversational confirmation after durable human authorization; `false` names rule authority and must not invent that confirmation. Counter-review reads the exact retained post-change repository snapshot; source bodies and generated diffs do not consume the sealed control-envelope cap. A residual `ENVELOPE_OVERFLOW` therefore concerns compact declarations or mandatory pinned evidence and should be diagnosed on that basis—not treated as automatic proof that source files or the approved phase must be split.
...
- **Reviewer substitution is explicit and request-scoped.** A dispatching `review` offer is the one action whose named `review-dispatch` submission is optional. When a configured reviewer route is unavailable, the skill asks the human for a substitute route and reason; it never chooses one itself. The server validates the supplied model, effort, and optional provider like configured routing, binds the override to that request and review run, and records the reason and displaced route in evidence without editing task config. The review remains mandatory, and an eligible no-wait result may advance without a later presentation while retaining that evidence.
- **Returned authority, then the exact commit or hand-off.** A matched approval rule opens `artifact-approval`, `design-approval`, or `commit-authorization`; every returned gate requires explicit human judgment. An eligible no-wait settlement can instead yield direct milestone, commit, or successor facts after the same mandatory review. Clients never invent either branch. Implementation requests a separate conversational confirmation only when its returned flag is `true`; migration audit and every safety or exception presentation remain unconditional human boundaries.
