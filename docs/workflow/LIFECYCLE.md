...
The optional config section has two lists. `subjects` accepts `prd`, `design`, `phase-design`, and `phase-impl`. `content` entries each carry repository-relative path globs and inspect the paths a result changed, including governing documents co-produced by later phases; with writable secondaries each glob is matched against every changed repository's own relative paths, so a glob cannot address a secondary by name. Matching is whole-path, case-sensitive, and `/`-segmented: `*` and `?` stay within one segment, while `**` spans zero or more complete segments (`**/*.sql` therefore matches both `a.sql` and `db/a.sql`). An absent or empty section records `wait:false`.
...
2. **counter_review** — one semantic action dispatches the configured general reviewers and, for phase design/implementation, the dedicated test specialist. Constitution review runs alongside them when active rules exist. Each child receives sealed context and the same read-only repository snapshot. Implementation review covers declared outputs and current post-change behavior; unchanged files and context-only repositories are supporting evidence. Fresh V5 evidence records explicit issues/no-issues outcomes and concise feedback with server-owned provenance, without deriving approval or a finding taxonomy. A failed sibling leaves received feedback visible but does not complete the review.
3. **triage** — the working AI reads the reports and submits a reasoned finish, revise, or escalate response. Revise names the previous reviewers needed to verify the changes; it returns a separate production entry before any editing. Finish settles feedback without requiring unanimous agreement. Escalate carries the question to the human.

Constitution evaluation remains separate. Failed or uncertain rules require remediation or the existing exception path, while matched approval triggers still require human judgment. Plan-alignment concerns in fresh reports are ordinary feedback, not a separate exact-census blocker. Archived structured reviews retain their original semantics.
...
**The working AI ends the feedback loop.** Follow-up dispatches only selected previous reviewers with their earlier report and verification request. Initial review still runs the configured roster. The server retains completed-round identities and checks current-subject authority, but does not enforce per-finding closure.

For archived structured review, `status.evidence.findings` retains each finding and disposition for audit, but ordinary human approval is not used to triage model polish. Rejected non-material observations stay out of the approval agenda. An envelope gap is disclosed there only when it prevented a material judgment; an `attempts-exhausted` gate instead presents the unresolved material defect and asks whether another revision is warranted. Only the authenticated repeated-review shape adds the separate push-through choice over the exact accepted occurrences.

Archived V1–V3 retain their native finding and alignment contracts. Archived V4 prose remains readable without inferred outcomes. Fresh V5 records explicit reviewer outcomes, concise feedback, and a working-AI response, without converting old evidence.
...
### Archived structured review: continuing after repeated accepted findings
...
Current report-based reviews expose `revise-minor` at every producing stage. The orchestrator may batch localized corrections, including a small clarification of established intent, without another reviewer call. It enters production before editing, runs applicable automated checks, and declares the actual diff as `minor` or `significant` in `review_revision` on the work result. Minor uses the authenticated one-hop predecessor path below, including phase-design effort evidence and implementation documentation/comment changes. Significant supplies no predecessor and returns to review. Coordinated rewrites, new requirements, behavior changes, and changed verification commitments require review. Required approvals still cover the final bytes, and minor revisions do not consume another review round.

For archived structured review, when a round's only accepted findings are `accepted-editorial`, the producer applies exactly the recorded revision intents and records produce again — and **nothing re-runs**. The revised artifact declares a server-validated, strictly one-hop `editorial_predecessor` link — `{subject_digest, input_fingerprint, triage_result_digest}` naming the exact reviewed bytes, their inputs, and the triage round that authorized the hop. The retained reviews *and* the constitution verdict stay bound to the declared predecessor for that one hop, and the eventual human gate presents the predecessor→final diff with an explicit disclosure that the evidence evaluated the predecessor bytes. A plain `accepted` disposition anywhere in the round still forces full re-entry — the editorial path exists only for rounds that are editorial through and through.
...
Only phase design adds the effort assessment. Review captures the current plan and hazards, plus the effective implementation preferences and benchmark catalog. The configured reviewer assesses remaining reasoning under V5 and returns a difficulty category with an explanation. The server deterministically chooses the cheapest enabled model meeting the configured threshold; defaults are 19/30/40/50 percent on Terminal-Bench v4.0. File counts and mechanical workload do not raise difficulty. The producer may assess useful design feedback through existing review/triage or reopen actions; the explanation does not change authority.

Effort selection has no fixed-point authority. It cannot create findings, blockers, re-entry, attempt exhaustion, or a human gate. An invalid or failed assessment uses the bounded-reasoning threshold and the captured preferences. A score shortfall is disclosed with the best available enabled model above the minimum; no eligible model or unusable selection data yields unavailable advice. Existing recommendations stay fixed when settings change. Archived blocker-shaped evidence remains readable and cannot change action selection.
...
Five total completed review rounds are allowed by default. Production entries, transport retries, and session restarts do not spend additional rounds. A finish response on the fifth round can continue through settlement and commit authorization. Requesting another round requires human direction. Legacy pre-history evidence conservatively retains attempt-based accounting until current evidence establishes completed-round history.
