...
The phase-completion signal normally fires from **triage-succeeded**: once triage closes the fixed point and explicit approval is recorded, the phase can advance. Accepted-editorial re-entry can re-establish the reviewed fixed point at **produce-succeeded** using its exact retained predecessor evidence and may record a new rule settlement. A human-requested simple revision is different: it records no settlement, cannot clear an accepted material finding, and always returns to the human for approval of the final bytes. `advance-phase` and `complete-task` are executable actions, not reports: the semantic surface recomputes status and derives the successor before composing the operation. PRD and design boundaries re-authenticate human approval; design also refuses to advance until Git proves the exact task-local milestone commit carries the matching decision archive and complete reviewed document set.
...
- **Every commit has exact durable human authority in the current rollout.** A document gate binds its task root, target, baseline, message, and complete reviewed document set. Implementation `commit-authorization` binds the retained final diff, exact target ref, baseline, message, and sorted paths. Approval returns the exact commit facts; implementation carries `requires_human_confirmation: true` — the client stages exactly the authorized paths, shows the human the staged diff and exact message, obtains explicit confirmation, creates the commit itself, and calls read-only `archflow_status` so the server observes proof. A rule settlement does not replace these facts before the constitution amendment. Because a design milestone covers the whole task directory, an unrelated task document sitting in it would ride along unreviewed; status refuses the commit and identifies the obstruction.
