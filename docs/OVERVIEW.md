...
Approval and advancement are intentionally two durable facts. Document boundaries (`prd`, `design`, and `phase-design`) fail closed unless the advance re-verifies an authenticated `artifact-approval` for the current produced bytes; an approval for a predecessor or stale digest cannot move the task. This hand-off adds status metadata and a composer kind, not a new durable-state or MCP schema, so existing tasks need no migration and a task stranded after approval can resume normally.
