...
Phase-design review has one additional best-effort child: the configurable effort reviewer. It assesses reasoning remaining after design as routine, bounded, hard, or exceptional, with a concrete explanation. File count, copying volume, security labels, and expensive tests alone do not raise difficulty. Optional repository hazards are context, not automatic floors. The server then chooses an implementation model deterministically from enabled profiles, Terminal-Bench v4.0 scores, and subscription-cost priorities.

The task's `implementation` config controls enabled profiles, cheapest-first cost priority, minimum score (19%), and difficulty thresholds (19/30/40/50%). Defaults enable GPT, GLM, and Gemini and select GLM 5.3 Flash for routine/bounded reasoning, GLM 5.3 max for hard work, and Astra high for exceptional work. The editable catalog in `assets/implementation-models.yaml` also retains opt-in Muse and Claude profiles. Reviewer failure uses bounded reasoning; a capability shortfall recommends the best enabled score above the minimum with an explanation, while unusable selection data or no eligible model produces unavailable advice. Captured evidence fixes existing recommendations; settings changes affect future assessments. Advice never changes the server-derived action or records the actual producer route.
