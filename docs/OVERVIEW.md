...
Approval and rule evaluation are intentionally separate durable facts. When a clean document fixed point is established, the same transaction reads and evaluates the live task config once and records a settlement for both outcomes. A waiting settlement preserves the exact subject/path reason shown at the ordinary `artifact-approval` or `design-approval`, even after later config edits. During the current staged rollout, neither conclusion is approval authority: every document still requires explicit human approval, and design advancement proves the exact task-local milestone commit from that authenticated decision. Implementation `commit-authorization` also remains mandatory and returns exact commit facts with `requires_human_confirmation: true`; the client stages the authorized paths, obtains explicit confirmation, creates the commit, and read-only status observes proof. Autonomous settlement consumption is deferred until the constitution and hard rules are explicitly amended in the same reviewed change.
