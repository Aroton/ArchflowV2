...
Approval and advancement are intentionally separate durable facts. PRD re-verifies `artifact-approval`; task design and phase design re-verify the combined `design-approval`, then prove its exact task-local milestone commit before advancing. That one human decision covers the document, every constitution conflict or trigger in the review, and commit authorization. It never creates a second confirmation prompt. Existing open legacy design `artifact-approval` gates remain valid under their recorded contract and do not gain retroactive commit authority.
