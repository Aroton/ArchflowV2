...
Two client loops coexist during the API migration. PRD, task design, phase design, and phase implementation use `archflow_status` for one reconciled, read-only view and `archflow_apply` for exactly one server-offered action. The offer hides revisions, digests, gate bindings, and request composition while keeping the client responsible for authored production and triage submissions. `/archflow-status` still uses `archflow-local status` / `build-request` and the four low-level tools. Git also remains client-owned: the semantic façade returns exact authorized commit facts — for an implementation, the plural path set, message, target ref, and baseline under `requires_human_confirmation` — and the client stages those paths, obtains the explicit confirmation, and creates the commit itself; the server never stages or commits repository bytes.
...
The counter_review step is one tool call and up to two server-run dispatches: the rubric review (read-only checkout at a pinned commit) and, only when the pinned constitution has active rules — the server decides, never the agent — the constitution review (sealed envelope, no checkout). Both results commit in one atomic transaction. Design subjects may be compound so planning can correct its parents: task design binds `design.md` with current `prd.md`, and phase design binds its phase document with current `design.md` and `prd.md`. The complete set is reviewed, approved, retained across revisions, reconciled, and commit-verified together. Self-review is not a durable step: the producing agent reviews its own draft as ordinary sub-agent work inside produce, before the artifact's bytes are ever recorded. Triage has three dispositions: `accepted` sends the work back into produce; `accepted-editorial` (non-blocking wording/formatting fixes only) lets the artifact be revised under a server-validated one-hop predecessor link with **nothing re-run** — retained reviews and the constitution verdict stay bound to the predecessor, disclosed at the gate; `rejected` requires a written rationale. Constitution verdicts are never triaged. At PRD and implementation they can still require a distinct constitution gate; at task-design and phase-design completion their findings are folded into the one final `design-approval` gate.

Human gates are deliberately not protocol consoles. A read-only preview composes a title, plain-language summary, direct question, material evidence, and labeled choices; skills present that conversationally and keep IDs, hashes, JSON, paths, and error codes in the diagnostic layer. After the human answers, the preview digest and nested choice/reason can travel in one bounded MCP call, whose connected handler records and resolves the decision synchronously; the semantic façade instead opens the gate from a summary and settles the returned decision through nonblocking archive-and-settle substeps. The server-dispatched review has already run automatically before the gate. If the human changes the work, the producer classifies the resulting diff: a simple wording or formatting change may reuse review evidence for one hop but still needs approval of the final bytes; a significant change resets the attempt counter and automatically starts a fresh counter-review and constitution-review cycle. The human can override either classification, with the override recorded.

Approval and advancement are intentionally separate durable facts. PRD re-verifies `artifact-approval`; task design and phase design re-verify the combined `design-approval`, then prove its exact task-local milestone commit before advancing. That one human decision covers every document in the compound subject, every constitution conflict or trigger in the review, and commit authorization. Implementation `commit-authorization` likewise resolves through the semantic surface: approval returns the exact commit facts bound to the retained final diff with `requires_human_confirmation: true`, the client stages exactly the authorized paths, obtains the explicit confirmation, and creates the commit itself, and read-only status observes the proof before the successor appears. Input-free `archflow-local commit` performs an authorized commit only on the legacy low-level path, preserving unrelated index/worktree state. Existing open legacy design `artifact-approval` gates remain valid under their recorded contract and do not gain retroactive commit authority.
