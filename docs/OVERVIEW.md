...
Approval and advancement are intentionally separate durable facts. PRD re-verifies `artifact-approval`; task design and phase design re-verify the combined `design-approval`, then prove its exact task-local milestone commit before advancing. That one human decision covers every document in the compound subject, every constitution conflict or trigger in the review, and commit authorization. Implementation `commit-authorization` likewise exposes a nonhuman `commit-phase` action bound to its retained final diff. Input-free `archflow-local commit` performs either authorized commit without a second confirmation prompt and preserves unrelated index/worktree state. Existing open legacy design `artifact-approval` gates remain valid under their recorded contract and do not gain retroactive commit authority.
