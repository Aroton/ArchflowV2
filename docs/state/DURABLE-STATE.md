...
The pipeline remains `produce → counter_review → triage`; accepted findings return to produce, and successful triage advances only after required human gates. Gate resolution and phase advancement are separate state commits. A composed gate call is bounded: its authenticated request may carry the human's choice and reason after a digest-bound preview, and the connected handler archives and resolves it in the same call rather than polling a disposable `gate.decision` file; the semantic façade instead archives the decision immutably and settles it in a separate nonblocking substep. Design adds a third recoverable fact between gate and advancement: the task-local milestone commit authorized by the combined gate. Status exposes `commit-artifacts` until Git proves that exact commit, then exposes the successor and the producer composes the judgment-free `advance` request. Implementation similarly exposes nonhuman `commit-phase` after `commit-authorization`; that authorization is the sole human confirmation, and design milestone commits require no second prompt because the one `design-approval` explicitly authorizes them. Input-free `archflow-local commit` re-derives the current authority and commits only its exact paths and message, preserving unrelated index/worktree state. If the session ended in between, the same skill resumes the pending commit or the exact destination skill completes the authenticated hand-off. No code is written before phase-design approval.
