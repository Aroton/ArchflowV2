...
Approval-rule settlements deliberately live inside this one state root. Each entry binds a producing phase instance, subject digest, config digest, settlement revision, and the full evaluated conclusion: no wait, or a wait with the exact subject/path match that triggered it. Both outcomes are written atomically with the transition that first establishes a clean fixed point, so later mutable config cannot rewrite the recorded match. During the staged rollout a settlement is presentation evidence only, not approval authority. There is no parallel settlement document, authority directory, or independently registered schema root that could drift from state.
