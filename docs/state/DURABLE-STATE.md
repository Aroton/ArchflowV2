...
**Explored:** 2026-08-20 · **Commit:** `3030d3d` · **Covers:** `src/state/`, `src/repository/`, `src/init/`, `src/local/`, `src/mcp/handlers/semantic.ts`
...
Historical approvals and rule settlements remain readable but are ineligible after a restart affecting their producer phase. Approval consumers enforce that cutoff for authority; settlement presentation selection enforces it so reproduced identical bytes cannot revive a stale trigger explanation.
...
The pipeline remains `produce → counter_review → triage`; accepted findings return to produce, and the clean fixed point atomically records what project rules evaluated. In staged Phase 4 both rule conclusions still open the ordinary explicit human gate for documents and implementations. A waiting implementation content rule adds frozen per-path operation and byte-delta evidence to that same `commit-authorization`; an unmatched TypeScript-only change still opens it without content-trigger details. Constitution adjudication, migration adoption, and the other safety/exception gates also remain human-only. Gate resolution and phase advancement are separate state commits. A semantic gate decision archives the human's choice and reason immutably and settles the gate separately; a disposable decision file is recovery machinery, never the protocol. Design adds a third recoverable fact before advancement: the exact task-local milestone commit authorized by the authenticated combined gate. Status exposes `commit-artifacts` until Git proves that exact commit, then exposes the successor. Implementation submit-work carries only the client-owned declaration; the server derives manifests, digests, scans, accounting, and verification evidence. After `commit-authorization`, status returns exact commit facts with `requires_human_confirmation: true`; the client obtains the separate explicit confirmation, creates the commit, and read-only status observes proof. No code is written before phase-design approval.
...
Document boundaries fail closed. A transition out of `prd`, `design`, or `phase-design-N` re-authenticates the phase-appropriate human approval for the current result and subject bytes; neither settlement conclusion supplies exit authority before the constitution amendment. For either design stage the subject includes the primary document and its parent projections. Human approval binds target ref, baseline commit, deterministic message, policy findings, and eligible waivers. Advancement independently proves its commit is the direct child of that baseline, touches only `.archflow/tasks/<task>/`, contains every document in the result plus state, and carries the archived request/decision pair. The task root must otherwise be clean. Existing legacy design `artifact-approval` archives remain usable under their recorded contract and are not treated as commit authority.
