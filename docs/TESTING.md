...
- Durable structural and semantic corpora, per-shape validation, gate/error/supplemental exhaustiveness, state `last_transition`, implementation verification evidence, and result manifests: `durable-contract-surface.test.ts`, `durable-structural-corpus.test.ts`, `durable-semantics-corpus.test.ts`, `durable-gate.test.ts`, `durable-gate-validation.test.ts`, `durable-state-validation.test.ts`, `durable-result-manifest-validation.test.ts`, `gate-error-supplemental-exhaustive.test.ts`.
