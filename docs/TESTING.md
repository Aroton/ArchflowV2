...
- Approval-rule coverage pins subject/content parsing, whole-path glob semantics, active template defaults, both durable settlement conclusions, numeric ordering across revision digit boundaries, restart cutoffs, persisted trigger presentation after config edits, waiver resumption boundaries, exact v2 authority, and fail-closed v1/divergent-policy behavior. Implementation content cases also pin complete per-path operation and exact signed byte deltas, independent old/new rename-endpoint matches, overlapping rename-away-plus-add-old effects, and deterministic ordering. Simple human revisions are pinned to final-byte approval and cannot suppress accepted material findings. The structural corpus proves settlements remain owned by `task-state` rather than gaining a separate schema root.
...
- Semantic implementation journeys (`semantic-implementation-journeys.test.ts`, `semantic-implementation-completion-journeys.test.ts`) cover the impl tier on the same shared harness (`test/helpers/semantic-journeys.ts`): the small client-owned work-result declaration with server-derived manifests and verification evidence, findings/no-findings review and triage, remediation through the separate `revise`, conditional commit authority, request-changes and exceptional gates, submission-shape refusals, hostile offer reuse, and both endings — the offer-free `start-next-skill` successor report at a non-final phase and the owned `finish-task` at the planned final phase. Default scaffolding authenticates the shipped v2 constitution. Default PRD/design subject rules and SQL implementation content still produce `wait:true` presentations; a TypeScript-only implementation reaches exact commit facts with `requires_human_confirmation:false`, while a human-authorized implementation returns `true` and retains the separate client-held confirmation. The document suite also proves autonomous milestones when subject triggers are deliberately removed. Explicit v1 and divergent-v2 fixtures remain fail-closed, and migration, policy, simple-revision, and safety/exception journeys remain human-required.
