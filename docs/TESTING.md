...
Specific-reviewer coverage is layered rather than measured by a percentage. Unit and contract tests pin phase-specific invocation legality, route precedence and Luna/xhigh fallback, stable assignments, non-overlapping criteria, exact primary upstream censuses including zero upstreams, strict general/test fields, server attribution, exact archive ownership, roster resizing, bounded legacy confirmation, and judgment-only constitution slots. Integration tests cover initial and remediation dispatch, no-active-rule alignment, typed upstream-authority failures before dispatch, sibling retention, policy parity across fresh/archive cohorts, ledger supersession, and exact `test-reviewer` failure attribution. Adapter tests send the same valid 300 KiB envelope through all three transports and prove no argv element approaches the 131,072-byte process limit.
