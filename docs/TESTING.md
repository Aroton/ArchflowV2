...
- `terminal-journey.test.ts` installs tracked `dist/` into a scratch home and exercises installed `archflow-local`/`archflow-mcp` slices including initialization, snapshot caps, dirty-worktree replay, maintenance, secret rejection, and legacy upgrade.
