...
The Zod parsers are the single runtime shape authority. The 36 committed schemas under `schemas/v1/` split 34/2: thirty-four are *generated from* the Zod sources by `npm run generate:schemas` (manifest in `internal/schema-generation.ts`, one plan module per shape group — leaf, durable, gate, errors, mcp-tools), and `npm run check:schemas` re-renders them in CI and fails on any byte drift, so the committed documents cannot disagree with the code. The two release schemas (`release-manifest`, `release-legal-review`) stay hand-written forever: they describe the release payload itself, are consumed only by `scripts/release-support.mjs`, have no Zod source, and the generator refuses to emit them.
