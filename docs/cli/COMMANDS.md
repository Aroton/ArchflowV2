...
```mermaid
flowchart LR
    J["Judgment only:<br/>findings, dispositions,<br/>rationales, summaries, decisions"] --> BR["build-request<br/>kind: initialize | produce | running | advance | restart |<br/>triage | counter-review | gate | waiver"]
...
- `restart` records an explicitly requested backward move to `prd`, `design`, or an earlier phase design. It is legal only for an active nonterminal task with no open gate and requires a human-readable reason. Git and worktree bytes stay untouched; the server archives superseded result references, clears target-and-downstream current authority plus active waivers and pending human revision, and enters the target at `produce: running`.
...
- `counter-review` takes no rubric or artifact-path facts. The composer derives the artifact path from the durable phase, and the server selects the immutable rubric for that phase kind. It does accept an optional `route_override` — a human's substitute reviewer and/or adjudicator route plus their reason — which it passes through for the server to validate; use it only to get past a reviewer CLI outage.
...
- `gate` composes a pending constitution gate mechanically for PRD and implementation. For task design and phase design it instead composes one `design-approval`, binding every constitution finding, eligible waiver, current subject, target ref, baseline commit, and commit message. Post-milestone projection drift composes a `baseline-adoption` decision ahead of any phase gate (a restore choice is refused here when no retained manifest holds the recorded bytes). Otherwise it picks the mandatory kind from the phase (`phase-impl` → `commit-authorization`, PRD → `artifact-approval`). Its payload is `{kind:"gate", summary, preview_digest, decision:{choice,reason}}`: the agent first runs `gate-preview`, presents that human-readable snapshot, asks once, and then carries the answer into `build-request`. The composer recomputes the preview and refuses stale bytes or a choice not offered by it.
- `waiver` uses the same bounded protocol after a `waiver-requested` origin decision. Its payload adds the archived `origin` and the requested `rationale`; the composer authenticates that origin and rejects a stale preview before staging `archflow_waiver`.
