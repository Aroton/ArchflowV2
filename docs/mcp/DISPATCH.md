...
- **`routing.ts`** — pure policy. Given the task config, phase, and role, it picks `{adapter, family, model, effort, provider?}`. The config roles are exactly the dispatched roles, `counter-reviewer` / `adjudicator` (`adjudicator` is simply the model/effort route for the constitution-review child); the producer is always the connected MCP host itself (derived from the initialize handshake) — it is never dispatched and has no config role. Family is inferred from the model name prefix (`claude-*` / `gpt-*`), except that a route naming a cc-switch `provider` forces the claude family so non-claude models (e.g. `glm-5.3`) can run through the claude CLI; pairing a provider with a `gpt-*` model fails `CONFIG_INVALID`. Families are **recorded, not enforced**: the shipped template defaults each reviewer to the host's opposite family, and a same-family reviewer is simply an explicit config choice. The adapter follows the resolved route; the host supplies only the no-route default (its opposite family).
