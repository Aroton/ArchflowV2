...
| Tool | What it does |
|---|---|
| `archflow_state` | The workflow's write path. Records that a pipeline step (`produce`, `counter_review`, `triage`) is running/succeeded/failed, optionally attaching a durable artifact. Its additive `planning_restart` operation arm routes an explicit strictly backward planning correction through the same state handler and bounded restart kernel. Runs a state transaction and returns the new revision. |
| `archflow_counter_review` | Selects the phase's immutable canonical rubric, assembles a sealed control envelope plus a read-only repository view, and dispatches it to the configured reviewer CLI (opposite family by default; optionally through a cc-switch provider). Implementation views are reconstructed post-change snapshots from the attested base commit and retained output bytes, so source size does not consume the envelope cap. When the pinned constitution has active rules (the server decides, never the agent), a second reviewer child receives its own sealed envelope and the same snapshot for constitution/drift review. Both results commit in one atomic state transaction; the result reports both: `{path, verdict, blocking_count, constitution, revision, request_digest}`, where `constitution` is `{status: "evaluated", path, constitution: pass\|fail\|uncertain, drift: aligned\|incidental\|material, triggers: […]}` or `{status: "not-run", reason: "no-active-constitution-rules"}`. `artifact_path` is now its only operation-specific full-request field; normal callers let `build-request` derive even that. |
| `archflow_gate` | Records and resolves a durable human gate decision against a subject digest. The bounded decision pair is optional and all-or-nothing: supply `preview_digest` plus `decision: {choice, reason}` and the connected handler resolves the already-presented decision in the same authenticated call, or omit both to open the gate and await the decision written through the interface. Task/phase-design `design-approval` combines document judgment, constitution findings, and task-local milestone commit authority. `migration-audit` is the corresponding single approval for a reviewed legacy import and binds its exact visible documents, resume point, and import commit authority; handles stale previews and supersession (`GATE_SUPERSEDED`). |
| `archflow_waiver` | Grants or denies a human waiver against a gate whose decision was `waiver-requested`, after re-verifying the archived origin gate. The same optional decision pair settles the waiver in one bounded call; without it the waiver opens and the decision returns through the interface. |
...
Every catalogue entry carries a purpose description. Low-level input schemas retain the plain-object projection that names their full-payload and staged-reference groups; semantic inputs are already compact plain objects generated from the semantic workflow contract. Gate and waiver full inputs carry the optional bounded decision pair as closed semantic request fields, and their advertised root stays a plain object even though it is decision-bearing, because some hosts erase a root combinator. The skills, not the tool listing, teach agents when to call what.
...
Human gates deliberately do not keep the MCP request open while waiting for another interface. The local `gate-preview` command first derives a conversational snapshot and canonical digest from current revision, phase, kind, subject, context, evidence, and offered choices. Commit previews also bind the exact authorized Git facts; implementation adds the retained diff digest, while design and migration bind the exact task root, baseline, target, and message because their final authority archive contains server-minted time/random provenance created only after the decision. After the human answers, `build-request` recomputes that preview and stages the choice and reason. The handler checks it again, opens and resolves the durable gate synchronously, and records provenance from the authenticated connected-host context (connection plus transport request identity). One human answer therefore produces one bounded tool call and one durable decision. Omitting the decision pair composes the same gate unopened; the semantic façade then carries the human answer through its nonblocking archive-and-settle substeps instead of a CLI round trip. The old `gate.decision` side channel remains reconstructible recovery machinery, not the normal protocol.
