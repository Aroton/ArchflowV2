...
The server selects deterministically from `implementation.enabled_profiles`. Default thresholds are 19/30/40/50 percent for routine/bounded-reasoning/hard/exceptional, with an absolute minimum of 19 percent. Among qualifying profiles, `cost_priority` wins (cheapest first: Muse, Z.ai, Google, GPT, Claude), then allowlist order within a cost group. If none meets the threshold, recommend the highest-scoring enabled profile above the minimum and disclose the shortfall; equal scores use the same cost/order tie break. No profile above the minimum, an empty allowlist, or missing/invalid selection data yields unavailable advice. These thresholds and priorities are configurable calibration choices, not benchmark guarantees.

The catalog is `assets/implementation-models.yaml` in the ArchFlow bundle; the task config template lists all profiles in comments. GPT, Muse, and Gemini profiles are enabled by default; Z.ai and Claude are opt-in. Unspecified effort remains omitted, and Fable's measured “with fallback” qualifier is preserved in the explanation. Adding a catalog profile and enabling it requires no model enum change. Defaults select Muse for routine and bounded reasoning, Astra low for hard work, and Astra high for exceptional work.
