...
2. **Counter-review call** — on the legacy loop, `archflow_counter_review` receives the artifact path and fingerprint, which `build-request` derives; on the semantic surface — now including phase implementation — one offered `review` action reaches the same seam, with the subject and pinned transcript derived entirely from durable authority. The server selects the rubric. Everything through step 6 happens inside this one call.
