...
**Explored:** 2026-08-20 · **Commit:** `3030d3d` · **Covers:** `src/review/`, `src/mcp/handlers/counter-review.ts`, `src/state/semantic-actions.ts`, `src/state/produce-subject.ts`, `src/state/evidence-results.ts`
...
8. **Rule settlement, then human decision** — the clean fixed-point transaction records what project `approval_rules` evaluated for the exact subject. A waiting task design presents `design.md` with its retained `prd.md`; a waiting phase design presents its primary phase document and both retained parent projections, together with the persisted rule match and every constitution finding. For implementation content rules, settlement freezes every matching path. Presentation joins that path set to the retained implementation output to reconstruct each operation and exact signed byte delta. Rename endpoints are matched independently, and overlapping effects such as renaming away and then adding at the old path remain separate, deterministically ordered evidence instead of being collapsed into a lossy path set. Only the archived gate summary is bounded: for a very large match it shows exact leading paths plus an omitted count so request construction cannot fail. The semantic presentation's operation/size `details` remains complete, reconstructed from the complete sorted path set in settlement and the retained output. Later config edits may still appear as an informational change notice, but cannot rewrite an already-settled trigger or its gate presentation. In staged Phase 4 both `wait:true` and `wait:false` still return to the ordinary human commit-authorization gate; autonomous consumption waits for the constitution amendment. After a requested change, the producer classifies the actual diff. Simple wording or formatting may keep predecessor evidence for one hop but always returns for approval of the final bytes and never clears an accepted material finding. Significant changes archive prior evidence, reset to attempt 1, and automatically dispatch fresh reviews. Uncertainty is significant; the human may override the classification in either direction.
...
- **The deviation is recorded where the human will see it.** Evidence produced under an override carries the reason together with the model and effort it displaced, and the rendered evidence gains a `Route Override` section. The approval gate therefore shows both which model actually reviewed and why it was not the configured one.
...
```mermaid
flowchart TB
    T[triage succeeds] -->|"task/phase design:<br/>policy or subject rule waits"| DA{{"one design-approval<br/>document + policy findings"}}
    T -->|"all policy rules pass,<br/>no project approval rule"| Gate["ordinary approval<br/>(current rollout)"]
