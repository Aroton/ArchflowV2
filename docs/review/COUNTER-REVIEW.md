...
8. **Rule settlement, then human decision** — the clean fixed-point transaction records what project `approval_rules` evaluated for the exact subject. A waiting task design presents `design.md` with its retained `prd.md`; a waiting phase design presents its primary phase document and both retained parent projections, together with the persisted rule match and every constitution finding. A very large content match shows exact leading paths plus an omitted count so the bounded gate summary cannot fail; the complete sorted match remains in durable settlement evidence for diagnostics. In the current staged rollout both settlement outcomes still return to the ordinary human gate; autonomous consumption waits for the constitution amendment. After a requested change, the producer classifies the actual diff. Simple wording or formatting may keep predecessor evidence for one hop but always returns for approval of the final bytes and never clears an accepted material finding. Significant changes archive prior evidence, reset to attempt 1, and automatically dispatch fresh reviews. Uncertainty is significant; the human may override the classification in either direction.
