...
The task's routing is byte-pinned at task creation and cannot be amended (see `../state/DURABLE-STATE.md`), which is right for reproducibility and wrong for a reviewer that is simply *down*. Without an escape hatch a logged-out or rate-limited reviewer CLI strands the whole task until the outage clears.

So one counter-review call may carry a `route_override`: a substitute `{model, effort, provider?}` for the counter-reviewer, the adjudicator, or both, plus the human's `reason` for it. It applies to that dispatch and nothing else — the pinned `config.yaml` and its digest are untouched, the next call routes normally again, and a role the override does not name keeps its pin.

Two things keep this honest rather than a way around the reviewer:

- **A substitute is validated exactly like a pinned route.** Same model-name rules, same family derivation, same cc-switch `provider` restriction, same per-adapter effort support. An override cannot express a route the config could not.
