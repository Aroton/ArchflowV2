...
8. **Rule settlement, then targeted authority** — the clean fixed-point transaction records what project `approval_rules` evaluated for the exact subject. A `wait:true` task design presents `design.md` with its retained `prd.md`; a waiting phase design presents its primary phase document and both retained parent projections, together with the persisted rule match and every constitution finding. For implementation content rules, settlement freezes every matching path. Presentation joins that path set to the retained implementation output to reconstruct each operation and exact signed byte delta. Rename endpoints are matched independently, and overlapping effects such as renaming away and then adding at the old path remain separate, deterministically ordered evidence instead of being collapsed into a lossy path set. Only the archived gate summary is bounded: for a very large match it shows exact leading paths plus an omitted count so request construction cannot fail. The semantic presentation's operation/size `details` remains complete, reconstructed from the complete sorted path set in settlement and the retained output. Later config edits may still appear as an informational change notice, but cannot rewrite an already-settled trigger or its gate presentation. Under the shipped exact v2 constitution, an authenticated `wait:false` settlement can instead produce direct document milestone, implementation commit, or successor authority when policy and safety checks are clean; the client follows only the returned action and never treats the settlement itself as authority. After a requested change, the producer classifies the actual diff. Simple wording or formatting may keep predecessor evidence for one hop but always returns for approval of the final bytes and never clears an accepted material finding. Significant changes archive prior evidence, reset to attempt 1, and automatically dispatch fresh reviews. Uncertainty is significant; the human may override the classification in either direction.
...
- **The deviation is recorded with the evidence.** Evidence produced under an override carries the reason together with the model and effort it displaced, and the rendered evidence gains a `Route Override` section. It is visible whenever a later presentation opens and remains auditable otherwise, but an eligible `wait:false` path may advance without any later human gate.
...
flowchart TB
    ...
    T -->|"implementation content<br/>rule waits"| CI{{"commit-authorization<br/>matched operations + deltas"}}
