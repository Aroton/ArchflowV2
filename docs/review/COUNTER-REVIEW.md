...
Fresh Review V5 evidence stores each round's explicit reviewer outcome and concise feedback, with server-owned reviewer, route, and reviewed-revision identity. The child returns exactly `{ "outcome": "issues_found", "feedback": "..." }` or `{ "outcome": "no_issues_found", "feedback": "Reviewed the changes; no actionable issues remain." }`. Both require nonblank text. A clean review is explicit signoff, never inferred from silence, arbitrary JSON, or an interrupted process. The outcome is the reviewer's judgment, not workflow approval; the working AI still owns triage. Archived Review V4 prose remains readable without inventing outcomes, and archived V1–V3 retain their original contracts.

Reviewers spend their work reading code/tests and reasoning about concrete failures. Mostly reading is an efficiency goal, not a quota. They return the location, failure, and consequence of actionable concerns; they do not quote whole files, narrate the investigation, write a separate review document, or inventory everything that is correct. Follow-up starts with the document or implementation revision diff, prior feedback, and the selected verification request, then inspects current code as needed to verify fixes and consequential regressions. Resolved findings need no repeated explanation. Existing human-readable review files are disposable server renderings of the same feedback, not additional model-authored artifacts.

Claude's optional numeric `usage` accompanies accepted feedback: input, output, cache reads/writes, thinking when reported, turns, duration, and estimated cost. Thinking is a subset of output, not an extra amount to add. Missing measurements remain unknown. Failure and current-phase success accounting lives in ignored diagnostics; accepted general/test feedback retains its measurements across phase cleanup and sibling retry. Standalone review returns measurements with each completed contributor. None of these estimates establishes provider billing or approval. Installed runtimes also keep a central, seven-day dispatch history under `~/.archflow/usage/` (or beside a custom install's `bundle/`). This history survives task cleanup and bundle replacement, contains no feedback or source text, and prunes expired records when the next dispatch completes.
...
Archived Review V1–V3 and pending gates remain readable without rewriting. Their original structured triage and alignment semantics apply until a fresh dispatch produces V5. Feedback responses do not build a disposition ledger; the small completed-round history remains for the budget.
...
Archived V1–V3 evidence and existing gates retain their original parsing and settlement rules. A fresh dispatch uses V5; old evidence is not rewritten.
...
Each numbered Markdown file under `.archflow/constitution/default/` or `custom/` is exactly one rule: frontmatter carries a stable `id`, a `version`, a `status`, and an optional `review_trigger` — specifically a human-review trigger, a condition under which the repository wants a person to decide at once; the shipped defaults declare two (below) — and the prose body is the normative text. Rule IDs are append-only — content changes bump the version, deprecation replaces deletion. Every active rule receives automated compliance review with or without that field; omitting it removes routine user approval, not AI review. The effective registry selects custom rules over defaults by ID, including deprecated replacements that suppress defaults. Duplicate IDs within a directory are invalid. Review sees each effective active rule once; both layers remain authenticated by the pinned policy digest. Historical flat policy stays readable. The eleven shipped rules are a good summary of the product's values:
...
After the working AI responds, status and request construction use the same policy-facts projection. Fresh V5 feedback has no automatic alignment verdict; current Adjudication V2 still supplies independent constitution judgments. Archived cohorts retain their native rule and alignment facts.
...
Fresh V5 views expose report text and authenticated reviewer identity, model, effort, route, and reviewed subject. The skill explains who reviewed the work and how the working AI responded, distinguishing current reports from previous-version and partial reports. The semantic renderer omits the retired `review_strength` and taxonomy projections when current V5 reports are present; it does not derive finding counts from prose.

Archived structured reviews retain their `review_strength` projection, including contributor and round counts where recorded. Those archive facts must not be presented as fresh V5 measurements. In either cohort, reviewer identity and the reviewed subject come from authenticated evidence rather than producer claims.
...
Phase-design review captures the current plan, repository hazards, implementation settings, and Terminal-Bench v4.0 catalog. The configured effort reviewer (Luna/xhigh by default) returns a difficulty category and a concrete explanation under `implementation-agent-selector-v5`. It assesses reasoning remaining after the reviewed design: routine instructions and known patterns, bounded local reasoning, hard unresolved algorithms or correctness interactions, or exceptional derivation. File count, copying volume, document length, tool-loop duration, and test runtime do not increase difficulty. Calling a tested mechanism does not inherit its invention cost.

The server selects deterministically from `implementation.enabled_profiles`. Default thresholds are 19/30/40/50 percent for routine/bounded-reasoning/hard/exceptional, with an absolute minimum of 19 percent. Among qualifying profiles, `cost_priority` wins (cheapest first: Z.ai, Google, GPT, Claude, Muse), then allowlist order within a cost group. If none meets the threshold, recommend the highest-scoring enabled profile above the minimum and disclose the shortfall; equal scores use the same cost/order tie break. No profile above the minimum, an empty allowlist, or missing/invalid selection data yields unavailable advice. These thresholds and priorities are configurable calibration choices, not benchmark guarantees.

The catalog is `assets/implementation-models.yaml` in the ArchFlow bundle; the task config template lists all profiles in comments. GPT, GLM, and Gemini profiles are enabled by default; Muse and Claude are opt-in. Unspecified effort remains omitted, and Fable's measured “with fallback” qualifier is preserved in the explanation. Adding a catalog profile and enabling it requires no model enum change. Defaults select GLM 5.3 Flash for routine and bounded reasoning, GLM 5.3 max for hard work, and Astra high for exceptional work.

Any difficulty-reviewer failure uses bounded reasoning with the same captured settings, without retry or a human boundary. The child explanation and server explanation are advisory: they never change review, triage, routing, offers, or authority. Changes to config or scores affect future assessments only. Retry snapshots are disposable task-local runtime data; intact snapshots retain the initial inputs, while missing/corrupt snapshots are recaptured without blocking. Completed evidence retains the full selection inputs and result, so status never rereads the catalog or recalculates advice.

Archived V1 assessments and V2/V3/V4-policy selections remain readable with their original meanings. Fresh schema-version-3 evidence records difficulty, rationale, selection inputs, recommendation, and reviewer/default provenance. Its recommendation must match deterministic derivation from its captured inputs. A policy update alone does not rerun a completed review; changed phase-design bytes require fresh assessment. See `../research/effort-research.md` for calibration limits.
