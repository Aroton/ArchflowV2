...
|---|---|---|---|
| VAL-01 | **blocked** | `docs/real-host-journeys.md` specifies the Claude-producer and Codex-producer terminal journeys, including exact human gates, two implementation phases, commit authorization, separate commit confirmation, and final-state capture. Prior normal-mode mechanics are covered by `test/integration/review-fixed-point-live-phase17.test.ts`, `test/unit/state-next-action-phase17.test.ts`, and the Phase 17 implementation log. | Neither operator journey can start through the installed MCP server. Claude Code 2.1.221 negotiates successfully but reports `connected · tools fetch failed`; a sanitized trace shows `notifications/initialized` followed by parameterless `tools/list`, then the installed server exits before responding. A direct three-frame replay does return the five-tool catalogue, so the remaining defect is real-host startup/catalogue interoperability rather than request shape. Codex 0.146.0 made two initialization attempts without reaching confirmed healthy status. The two journey evidence files therefore do not exist, and there is no terminal proof of either producer direction. |
...
spawns. ArchFlow selected `2025-11-25` in both responses. Negotiation evidence is therefore present,
but tool discovery is not healthy: Claude reports `tools fetch failed`, while Codex never reached a
confirmed healthy status. This blocks VAL-01 and leaves VAL-09's real pending-call timeout partial.
