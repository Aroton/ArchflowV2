...
| VAL-15 | **passed** | `test/unit/adjudication.test.ts` covers positive, negative, uncertain, missing, stale, failed, and digest-mismatched mechanism evidence. `test/unit/review-services.test.ts` proves triggered/uncertain gates and exact obligation resolution. `test/integration/mcp-adjudicate-constitution-gate.test.ts` proves task-branch constitution edits gate before dispatch and cannot fabricate success. | No outstanding boundary identified for constitution trigger/mechanism and task-branch edit handling. |
