...
| VAL-05 | **passed** | `test/integration/mcp-handler-state-replay-phase15.test.ts` proves a byte-identical authoritative result on exact replay; `test/integration/mcp-handler-counter-replay-phase15.test.ts` proves replay/recovery does not relaunch a reviewer. `test/crash/state-transaction.test.ts` proves receipt/result/state cut recovery, while the Phase 9 log records the real multi-process and crash-cut matrix. Against the installed launchers, `test/real-host/terminal-journey.test.ts` proves dirty-worktree exact replay plus all three restore-collision decisions: discard restores only the retained target, adoption requires a changed durable input fingerprint plus exact authority and rationale while preserving the human generation, and abort preserves workflow authority and collision bytes. | No outstanding boundary identified for the stated exact-replay and restore-collision condition. |
