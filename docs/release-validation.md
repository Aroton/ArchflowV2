...
suite in `test/real-host/terminal-journey.test.ts` was rerun from this current `dist/` and all six
...
- **VAL-05:** no current installed dirty-worktree exact replay with unrelated tracked and untracked
  files unchanged; no current installed exercise of discard-and-restore, adopt, and abort collision
  decisions.
- **VAL-06:** the current installed suite still does not exercise secret-bearing implementation
  output rejection before projection/state advancement or a safe maintenance-prune record. Its
  current snapshot run returns `SNAPSHOT_LIMIT` above the retained-task byte cap, while a result one
...
VAL-01 through VAL-06 are **not all passed**: VAL-01 is pending, VAL-02 is blocked, and VAL-05 is partial. Archforge work therefore
